...
whoFor: "a SOC/incident-response team that already runs Defender XDR and/or Insider Risk"
frameworks: ["GDPR"]
...
Full licensing detail: [Licensing matrix](/docs/licensing-matrix/). RBAC: [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive). Summary:
...
| Role groups (this scenario provisions) | **Data Security Investigations Admins**, **Data Security Investigations Investigators**, **Data Security Investigations Reviewers**, see §6 for the exact permission matrix |
...
| Automation identity (audit-trail script) | An Exchange Online PowerShell session (`Connect-ExchangeOnline`) held by an identity with the **View-Only Audit Logs** or **Audit Logs** role, [RBAC model §6](/docs/rbac-model/#6-exchange-online-dependency-the-most-common-permissions-gap) |
...
. A Microsoft 365/Office 365 license already covers the
...
- **Purge is source-scope-limited.** Even though an investigation's *scope* can include SharePoint and
 ...
 purge actions entirely. A buyer expecting DSI to purge an overshared SharePoint
...
- **Purging does not remove data from DSI's own storage.** A purge removes items from Exchange/Teams;
 ...
 billing) until the investigation itself is deleted, see `rollback.md`.
