...
whoFor: "an enterprise HR/Legal/Compliance function that wants to move from a purely"
frameworks: []
...
| Policy authoring role | **Communication Compliance** or **Communication Compliance Admins** role group | See §5, policy authoring is portal-only; these role groups also grant the **Communication Compliance** left-nav item itself |
| Reviewer role (this scenario) | **Communication Compliance Investigators** (full message content), not Analysts (metadata only) | See `design.md` §5 for why Investigators is the deliberate choice for a credible HR investigation. Cross-ref [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive) |
...
. This is a tenant-wide setting, not per-policy.
...
. Lowering review percentage is the last lever, not the first, for a
