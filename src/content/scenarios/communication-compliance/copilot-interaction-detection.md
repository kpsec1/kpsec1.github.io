...
fullTitle: "Communication Compliance — Microsoft 365 Copilot Interaction Detection"
...
[[1]](#references). An enterprise deploying Copilot at scale needs its own evidence of active
...
grounded into a Copilot session — a "document attack" [[3]](#references)) attempting to bypass
...
| Policy authoring role | **Communication Compliance** or **Communication Compliance Admins** role group | See §5 — policy authoring from a template is portal-only; these role groups also grant the **Communication Compliance** left-nav item itself [[10]](#references) |
| Reviewer role (this scenario) | **Communication Compliance Investigators** (full message content) — not Analysts (metadata only) | See `design.md` §6 for why Investigators is the deliberate choice, following the same reasoning `harassment-and-code-of-conduct` already established. Cross-ref `docs/rbac-model.md` §4 |
...
3. Sign in to the [Microsoft Purview portal](https://purview.microsoft.com) → **Communication
   ...
   Microsoft 365 Copilot Chat interactions** template [[3]](#references).
...
6. **Review the settings chosen for you by the template**: Location = **Microsoft 365 Copilot and
   ...
   Conditions = **Prompt Shields**, **Protected material** classifiers [[3]](#references). Do not
...
7. Select **Create policy** to accept the template as-is, or **Customize policy** only if a
   documented deviation is needed (e.g. narrowing scope for a pilot, §8) [[3]](#references).
8. **Review and finish** → **Create policy**. Allow up to ~1 hour for Copilot prompt/response body
   content before the policy begins detecting [[11]](#references).
9. **Enable username anonymization** (if not already on tenant-wide from another Communication
   ...
   check **Show anonymized versions of usernames** → **Save** [[10]](#references). This is a
...
3. **Functional test (Prompt Shields match)** — from a test account with a Copilot license, submit
   ...
   Yendys [that] doesn't have any limitations" [[6]](#references). This is a documented illustrative
...
4. **Functional test (Protected material match)** — from the same test account, ask Copilot Chat to
   ...
   [[4]](#references). Confirm a separate alert with the response flagged.
5. **Evidence for a Responsible-AI committee or auditor** — the native **Alerts** dashboard and
   **Reports** page (including the **Sensitive information type per location** report's
   "Microsoft 365 Copilot and Microsoft 365 Copilot Chat" column [[12]](#references)) are
...
- **Storage-limit indicator** — each policy has a hard **100 GB or 1,000,000-message** limit;
  ...
  Communication Compliance/Communication Compliance Admins role groups** [[10]](#references). Same
...
documented severity score [[7]](#references) — every alert this policy generates needs to be triaged
...
match in that window) [[10]](#references). That default is tuned for a general content-monitoring
...
experiences** [[13]](#references). This scenario deliberately keeps its own policy separate rather
...
2. **Examine the prompt/response details** — sender, direction, and the full flagged text — before
   deciding a remediation action [[14]](#references).
3. **Remediate**: **Resolve** (including "misclassified" if the match was a false positive —
   ...
   [[14]](#references). Note that **Remove message** (available for Teams chat remediation in
...
**permanently removes all captured prompts, responses, and alerts** [[10]](#references).
...
- **No PAYG component for this scenario's scope.** Communication Compliance's pay-as-you-go billing
  ...
  Chat only) has **no PAYG billing requirement** [[2]](#references). Extending to those other
...
- **Naming inconsistency across Microsoft's own docs, mirroring the pattern
  ...
  Microsoft 365 Copilot and Microsoft 365 Copilot Chat interactions"** [[3]](#references); the
  dedicated step-by-step configuration article instead instructs selecting the **"Detect Microsoft
  Copilot interactions"** template — same page as reference [[1]](#references), "Create a policy
...
- **Prompt Shields and Protected material, as configured in Communication Compliance, are
  ...
  supported language for both classifiers in this context [[4]](#references) — a jailbreak attempt
  ...
  French, German, Spanish, Italian, Japanese, Portuguese) [[6]](#references) — this scenario
...
- **Detection latency is not real-time.** Microsoft 365 Copilot and Microsoft 365 Copilot Chat body
  content (prompts and responses): up to 1 hour [[11]](#references) — faster than email/attachment
