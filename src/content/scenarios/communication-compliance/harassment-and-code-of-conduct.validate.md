...
#### `Test-CommunicationComplianceAuditTrail.ps1`
...
.DESCRIPTION
    ...
    1. AUTOMATED (hard pass/fail, contributes to exit code) - read-only, no tenant connection
       ...
       - No duplicate CompositeKey rows - proof the deploy script's merge-and-de-duplicate design
         (design.md Section 8) is actually holding across re-runs.
    ...
    2. MANUAL (printed as a checklist, never fails the script) - the policy's existence, scope,
       classifiers, reviewers, and the User-reported messages policy's reviewer reassignment, none
       of which have a documented read API (design.md Section 2). This is not a gap in this script
       - it is Communication Compliance's current product surface.
...
.PARAMETER AuditTrailCsvPath
    Path to the CSV produced by deploy/Export-CommunicationComplianceAuditTrail.ps1.
...
.PARAMETER PolicyName
    ...
    scenario's docs and deploy/policy/communication-compliance-policy-manifest.json.

.EXAMPLE
    ./Test-CommunicationComplianceAuditTrail.ps1 -AuditTrailCsvPath '../deploy/out/cc-audit-trail.csv'
...
.NOTES
    ...
    Sources: see deploy/Export-CommunicationComplianceAuditTrail.ps1 .NOTES for the Microsoft Learn
...
param(
    ...
    [Parameter()]
    [string]$PolicyName = 'Workplace Harassment and Code of Conduct - All Users'
)
...
if (-not (Test-Path -Path $AuditTrailCsvPath -PathType Leaf)) {
    Write-Host "`nCannot continue - file not found. Run deploy/Export-CommunicationComplianceAuditTrail.ps1 first." -ForegroundColor Red
    exit 1
}
...
$manualChecklist = @(
    "Purview portal > Communication Compliance > Policies: a policy named '$PolicyName' exists, status Active (not Paused/Error), scoped to Exchange Online + Microsoft Teams + Viva Engage."
    "Policy's conditions include the Discrimination, Harassment (may display as 'Targeted harassment'), Profanity, and Threat trainable classifiers, plus the custom keyword dictionary from deploy/policy/code-of-conduct-evasion-phrases.txt, matching deploy/policy/communication-compliance-policy-manifest.json."
    "Policy's Reviewers list matches deploy/policy/communication-compliance-policy-manifest.json's reviewers.placeholderMembers (replaced with the tenant's actual HR/Legal stakeholders) - not left at any default."
    ...
    "Communication Compliance > Policies > User-reported messages policy: reviewers have been reassigned away from the default Communication Compliance Admins/Global Admin fallback, per deploy/policy/communication-compliance-policy-manifest.json's userReportedMessagesPolicy."
    "Settings > Communication Compliance > Privacy tab: 'Show anonymized versions of usernames' is enabled, per deploy/policy/communication-compliance-policy-manifest.json's privacySettings."
    "Settings > Communication Compliance > Notice templates: a notice template exists for use with the 'Notify' remediation action, per deploy/policy/communication-compliance-policy-manifest.json's noticeTemplate."
    "Policies page: the policy's storage-limit indicator is not approaching 80/90/95% of the 100 GB / 1,000,000-message per-policy limit (README.md Section 11) - a policy that silently auto-deactivates at the limit stops generating alerts with no in-band warning to anyone outside the Communication Compliance/Communication Compliance Admins role groups."
    "If this file shows 0 policy-update events but the policy's configuration looks different from deploy/policy/communication-compliance-policy-manifest.json: the audit window may predate the change, or Audit (Standard)'s 180-day retention has already expired it (README.md Section 11) - do not assume 'no events in this file' means 'no changes ever happened.'"
)
