...
. Priority cleanup lets an organization reclaim that storage on a standing basis.
...
Full licensing detail: [Licensing matrix §7](/docs/licensing-matrix/#7-adjacent-product-family-microsoft-defender-for-endpoint--intune-device-control-scenarios). RBAC: [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive). Automation
...
| Creator/first-stage role | **Priority Cleanup Admin** (not included by default in Compliance Administrator; add manually) | [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive) |
...
| Conditional approver (eDiscovery Admin) | **eDiscovery Administrator** role; approval required **only if** an identified item is under one or more eDiscovery holds | [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive); policy creation fails with an error if this reviewer lacks the role |
...
| Feature toggle | Enabled tenant-wide by default; can be turned off entirely, same switch as Exchange | Portal: Data Lifecycle Management → Priority cleanup settings, no confirmed PowerShell equivalent found (§11) |
...
. Items already in the Recycle Bin can still be recovered from there within its
