...
#### `Test-PriorityCleanupExchangePolicy.ps1`

```powershell
#Requires -Version 7.0
<#
.SYNOPSIS
    Verifies the priority cleanup label, policy, and rule exist and are classified as priority
    cleanup objects, and reports the policy's current mode (simulation vs. enforced).
...
.DESCRIPTION
    ...
    property name on the returned object - Microsoft's reference does not publish one). Checks:
      ...
      2. The policy exists in the -PriorityCleanup-filtered policy list, with the expected
         ExchangeLocation(s), and reports Enabled/DistributionStatus.
    ...
    (search on the policy's Cleanup ID, or the PriorityCleanupTagApplied / PriorityCleanupDelete
...
.PARAMETER ConfigPath
    ...
    '../deploy/config/priority-cleanup-exchange.sample.json'.
...
param(
    ...
    [string]$ConfigPath = (Join-Path $PSScriptRoot '../deploy/config/priority-cleanup-exchange.sample.json')
)
...
Write-Host "Validating Exchange priority cleanup ('$($cfg.label.name)' / '$($cfg.policy.name)')..." -ForegroundColor Cyan
...
if ($policy) {
    $expectedLocations = @($cfg.policy.exchangeLocation)
    $actualLocations = @($policy.ExchangeLocation)
    $hasAllExpected = -not ($expectedLocations | Where-Object { $_ -notin $actualLocations -and $actualLocations -notcontains 'All' })
    Test-Check -Description "  ExchangeLocation covers the expected mailbox(es)" -Condition $hasAllExpected -Warn
    Write-Host "    Enabled: $($policy.Enabled) | Mode: $($policy.Mode) | DistributionStatus: $($policy.DistributionStatus)" -ForegroundColor Cyan
    if (-not $policy.Enabled) {
        Write-Host "    Policy is disabled or still in simulation mode - no items will be identified for deletion until enforced." -ForegroundColor Yellow
    }
}
...
Write-Host "  Use auditing (search the policy's Cleanup ID; PriorityCleanupTagApplied / PriorityCleanupDelete operations) to confirm actual deletions - see README.md Section 7." -ForegroundColor Cyan
