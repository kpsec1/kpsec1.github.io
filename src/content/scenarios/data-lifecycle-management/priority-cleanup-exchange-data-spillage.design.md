...
otherwise prevent it**. This scenario builds that control as code, for the
...
1. **Script the real mechanism, not a guess.** Priority cleanup is not a separate product surface, 
 "under the covers, priority cleanup uses retention labels with auto-apply policies",
...
deletion** sub-feature for that workload) is now built as its own sibling fragment,
...
- **`-MultiStageReviewProperty` as the approver mechanism.** `New-ComplianceTag`'s official
 ...
 eDiscovery admin). But no Microsoft-published example ties this
 ...
 the required role for their stage, so an incorrect mapping surfaces as a loud,
...
- **`RetentionDuration`/`RetentionType` for "delete as soon as possible."** The portal wizard offers a
 binary choice, delete ASAP, or retain for a period then delete, but the cmdlet
...
, but this is this scenario's inference, not a confirmed default.
...
already completed, matching Microsoft's own limitation notice.
...
- **eDiscovery search-and-purge as an alternative path**, Microsoft's own tip suggests purging
 ...
 items, to avoid showing end users the "Retention:... (-1 days)" message bar.
