...
whoFor: "an HR / records-management / compliance team that must retain a departed"
frameworks: []
...
. Applying the published label to content, and setting each item's Asset ID, is
...
| `IsRecordLabel` | `$true` | Locks labeled content as a record; mirrors Microsoft's own worked event-based example. Not a *regulatory* record, see `retention-labels-financial-records` for that stronger control |
