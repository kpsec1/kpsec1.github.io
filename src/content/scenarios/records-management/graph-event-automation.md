...
whoFor: "a records-management / platform-integration team that already has event-based"
...
. So a current-best-practice integration uses Graph. The obligations behind it are the
...
[Automation surface](/docs/automation-surface/) (surface 2, Microsoft Graph PowerShell SDK; surface 3, Graph REST).
Summary:
...
. For a high-volume integration, fire one event per business entity (per contract, per
...
. Firing is gated behind `-FireEvent` **and** `event.fire=true`, and always goes
