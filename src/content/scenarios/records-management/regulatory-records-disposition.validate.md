...
#### `Test-RecordsDisposition.ps1`

```powershell
#Requires -Version 7.0
<#
.SYNOPSIS
    Verifies the event-based records-disposition lifecycle: the event type, the event-based record label
    (KeepAndDelete + disposition review, bound to the event type), the publish policy, and the publish
    rule exist and are configured as expected.
...
.DESCRIPTION
    ...
    against the config file:
      ...
      2. The record label exists, is a record, uses RetentionAction=KeepAndDelete and
         RetentionType=EventAgeInDays, is bound to the event type, and (best practice) has a disposition
         reviewer.
      3. The publish policy exists and is enabled with at least one location.
      4. The policy's rule PUBLISHES the expected label (PublishComplianceTag).
      5. (Informational) reports any triggered events for the event type.
...
.PARAMETER ConfigPath
    ...
    '../deploy/config/records-disposition.sample.json'.
...
param(
    ...
    [string]$ConfigPath = (Join-Path $PSScriptRoot '../deploy/config/records-disposition.sample.json')
)
...
Write-Host "Validating records disposition ('$($cfg.eventType.name)' / '$($cfg.label.name)' / '$($cfg.policy.name)')..." -ForegroundColor Cyan
...
if ($label) {
    ...
    $hasReviewer = @($label.ReviewerEmail).Count -gt 0
    $wantsReviewer = ($cfg.label.reviewerEmail -and @($cfg.label.reviewerEmail).Count -gt 0)
    if ($wantsReviewer) {
        Test-Check -Description "  Disposition reviewer(s) set (current count: $(@($label.ReviewerEmail).Count))" -Condition $hasReviewer -Warn
    }
    else {
        Test-Check -Description "  No disposition reviewer configured - label auto-deletes at end of retention (config has none)" -Condition $true -Warn
    }
}
...
Write-Host "`n  Note: publishing can take up to 7 days to reach apps; a triggered event syncs to labeled content up to 7 days. Review pending disposals in the portal (Records Management > Disposition)." -ForegroundColor Cyan
