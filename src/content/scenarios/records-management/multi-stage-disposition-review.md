...
whoFor: "a records-management team (working with HR and Legal) at an organization that needs a"
frameworks: []
...
; FLSA requires **3 years** for payroll records and **2 years** for records underlying
...
| `AutoApprovalPeriod` | `30` days | Valid range 7-365, default 14 if set with no value; **silently advances/disposes a stage with no reviewer action**, see §8 |
...
- **`-ComplianceTagForNextStage`'s behavior is undocumented by Microsoft.** Its own published parameter
 ...
 placeholder. The Microsoft Graph records-management
...
, but this scenario does **not** assume PowerShell's parameter behaves identically.
