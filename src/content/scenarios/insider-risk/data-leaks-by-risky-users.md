...
whoFor: "a tenant that wants employment-stressor-triggered exfiltration detection"
...
. This template correlates that signal with **data-exfiltration
...
| **Microsoft 365 HR connector**, configured for disgruntlement/risk indicators, **OR** | Job level change, Performance review, and/or Performance improvement plan CSV data type(s) | At least one of this row or the Communication Compliance row below is required; both together is supported and recommended, §5 Step 4 |
| **Communication Compliance risk-signal integration**, dedicated policy | Selected as an option in the IRM policy-creation workflow itself | Auto-creates a dedicated "Detect inappropriate text" Communication Compliance policy, portal-only, no PowerShell surface for Communication Compliance policy authoring exists; §5 Step 5 |
...
| Role to create the HR connector | **Data Connector Admin** (or a role group that includes it) | [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive); distinct from the IRM policy-configuration role above |
...
([RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive)). No Defender for Endpoint role is needed anywhere in this scenario, 
 ...
 **Performance review**, and **Performance improvement plan** as the HR scenarios to import, and
 map columns per §6's reference (identical schema to the sibling scenario).
...
. Users sending 5 or more messages classified as potentially risky within 24
...
| Optional cloud indicators | Cloud storage (Box, Dropbox, Google Drive) / cloud service (Amazon S3, Azure) indicators | Requires Defender for Cloud Apps app connections + pay-as-you-go billing; **VERIFY at deploy time whether this template's policy-creation workflow actually offers this category**, §5 Step 6 |
...
- **Communication Compliance licensing, if that trigger and/or scoring path is used:**
 ...
 (E5/A5/G5, Purview Suite, or an E5 add-on) per [Licensing matrix §2](/docs/licensing-matrix/#2-master-capability--license-matrix), confirm this
...
- **Pay-as-you-go billing, if cloud storage/cloud service indicators are used**, 
 [Licensing matrix §2](/docs/licensing-matrix/#2-master-capability--license-matrix)'s "Cloud/GenAI indicators on non-M365 → PAYG (Data Security
