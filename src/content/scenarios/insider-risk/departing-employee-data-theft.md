...
whoFor: "any Microsoft 365 E5 (or equivalent add-on) tenant with an HR system that can"
...
Full licensing detail and citations: [Licensing matrix §2](/docs/licensing-matrix/#2-master-capability--license-matrix) (Insider Risk Management
...
| Role to configure policies/settings | **Insider Risk Management** or **Insider Risk Management Admins** role group | See §6 below and [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive), narrower **Analyst**/**Investigator**/**Auditor** groups exist for day-2 operation without policy-authoring rights |
...
(**Purview** → **Audit** → confirm status, or [RBAC model §6](/docs/rbac-model/#6-exchange-online-dependency-the-most-common-permissions-gap) for the underlying
...
5. **Content priorities** (optional but recommended): select the tenant's confidential/highly-
 confidential sensitivity labels so matching activity scores higher.
...
| Priority user group | None by default | See §8 for when to add one; limit is 10,000 users per group |
...
user-groups`), this sharpens alert severity for departing users in those
...
. Office/device indicators on M365 signals are covered by the base
...
- **No PowerShell or Graph write API for policy authoring.** As of this writing, Insider Risk
 ...
 library does not fabricate a cmdlet for them. [Automation surface §6](/docs/automation-surface/#6-cicd-and-unattended-execution-guidance);
 `design.md` §4/§6.
...
- **Client-secret auth for the HR connector, not this repo's usual certificate pattern.**
 ...
 library's certificate-first default ([Automation surface §3](/docs/automation-surface/#3-authentication-patterns-interactive-vs-unattended)). Store the secret in a
