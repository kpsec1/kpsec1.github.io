...
- **The base `Data leaks` template's own two triggering-event options, unchanged**, "User matches a
 ...
 custom thresholds). This is a materially different shape from the
...
- **`security-policy-violations-by-priority-users`'s population mechanism, unchanged**, a **priority
 ...
 for this template, with the same
...
- **Its own, independently-documented 1,000-actively-scored-user cap**, numerically identical to
 ...
 15,000, `Data leaks by risky users` at 7,500). §3 below grounds this
 ...
 **Users and groups** page, stated to appear "only if you choose the *Data leaks by priority
 users* template", a materially more precise, directly-fetched fact
...
. §2 goal 4 treats this as a first-class configuration step this
...
, calling it unmodified is the correct reuse, not a new script.
2. **Reuse `security-policy-violations-by-priority-users`'s priority-group candidate-resolution
 ...
 template's own, independently-confirmed cap too. This scenario calls
...
. Nothing in the `security-policy-violations-by-priority-users`
...
5. **Ground the DLP-alerts indicator's current supported/unsupported workload list precisely,
 ...
 BI**, the Microsoft 365 Copilot exclusion
...
6. **Correct, don't repeat, the cross-template cap-sharing ambiguity a same-family sibling carries.**
 ...
 policy template**", i.e. per exact template, not per family. §3
...
7. **Don't fabricate a policy- or priority-user-group-authoring API.** Same finding as every other
 ...
 and the priority user group are portal-only ([Automation surface §6](/docs/automation-surface/#6-cicd-and-unattended-execution-guidance)).

## 3. The two caps, precisely stated for this exact template

| Cap | Value | Scope | Source |
|---|---|---|---|
| Priority user group membership | **10,000** members | Per priority user group object (a tenant-wide setting, not per-template) | |
...
, and the Limits table lists each template as its own row with its own
number. What remains genuinely unconfirmed, identical to the sibling's
...
directly-fetched **Office indicators** description) plus cumulative
exfiltration detection (default-on for this template family), with two
...
|---|---|---|
| Triggering event | Reuse the base `Data leaks` template's own choice: DLP-policy match (primary, worked example) or exfiltration activity (documented alternative) | Grounded directly against Microsoft's own "Data leaks by priority users" template description and Step 6/Step 12 of the configuration guide, not the fixed, single-trigger shape of the `security-policy-violations-by-priority-users` sibling |
