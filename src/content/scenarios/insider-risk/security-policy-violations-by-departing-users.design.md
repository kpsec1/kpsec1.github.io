...
Endpoint indicators **(preview)**" (full citations in README.md §12).
 ...
 *Data theft by departing user*, *Security policy violations by departing user*, *Data leaks by
 ...
 the same HR-connector dependency, the tenant configures **one** HR
...
2. **Be explicit about the one genuinely new prerequisite:** an active Microsoft Defender for
 ...
 Endpoint **Plan 2** bundled at no incremental cost; a tenant that
...
3. **Don't fabricate a policy-authoring or advanced-features API.** As with the sibling scenario,
 ...
 ([Automation surface §6](/docs/automation-surface/#6-cicd-and-unattended-execution-guidance)). This build additionally searched for a documented Graph or
...
4. **Ship the one export capability this template's specific correlation model makes genuinely
 ...
 microsoftInsiderRiskManagement`), this scenario's
...
5. **Disclose, not assume, the one part of goal 4 that isn't independently confirmed.** Microsoft
 ...
 the same attacker" and separately documents that Insider Risk
 Management alert data is "shared with Defender XDR unified alert queue"
, but no worked example in Microsoft Learn was found during this
...
. Alerts from Defender for Endpoint are imported **daily**, and Microsoft
...
Resolved), `README.md` §11 carries this forward as an
...
| Cross-policy-template disambiguation for exported alerts | Not attempted, documented as an open gap | The Graph `alert` resource's `alertPolicyId` field is populated "when there's a specific policy that generated the alert", but Microsoft doesn't document how to map that GUID back to a named Purview IRM policy via any API, so a tenant running more than one "Security policy violations…" family template in parallel cannot programmatically tell which one produced a given exported alert. `README.md` §11 states this plainly rather than fabricating a lookup. |
