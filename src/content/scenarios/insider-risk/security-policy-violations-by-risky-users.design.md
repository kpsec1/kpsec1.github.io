...
. This is the fourth and final member of the "Security policy
...
1. **Ship the one genuinely new, scriptable capability this template's HR requirement makes
 ...
 pattern. Reusing the sibling script unmodified would either silently
...
3. **Get the indicator-vs-trigger distinction right for THIS specific template, not by analogy to
 ...
 users". For this template, Communication Compliance integration is
...
, and IRM policy authoring remains portal-only across the whole
 family ([Automation surface §6](/docs/automation-surface/#6-cicd-and-unattended-execution-guidance)).
...
| Communication Compliance dedicated policy | Auto-created by the IRM policy-creation workflow's own option | No, portal-only, no PowerShell surface exists for this product at all |
