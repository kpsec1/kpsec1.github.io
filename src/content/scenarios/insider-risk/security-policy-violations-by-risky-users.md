...
whoFor: "a tenant that already runs (or is deploying) the base \"Security policy"
...
. Correlating that stressor signal with a security-control violation on the same
...
Full licensing detail and citations: [Licensing matrix §2](/docs/licensing-matrix/#2-master-capability--license-matrix) (Insider Risk Management and
...
| **Microsoft 365 HR connector**, configured for risk indicators, **OR** | Job level change, Performance review, and/or Performance improvement plan CSV data type(s) | At least one of this row or the Communication Compliance row below is required; both together is supported and recommended, §5 Step 5 |
| **Communication Compliance risk-signal integration**, dedicated risky-user policy | Selected as an option in the IRM policy-creation workflow itself | Auto-creates a dedicated "Detect inappropriate text" Communication Compliance policy, portal-only, no PowerShell surface for Communication Compliance policy authoring exists; §5 Step 6 |
...
| Defender for Endpoint → Purview alert sharing | "Share endpoint alerts with Microsoft Compliance Center" advanced feature, enabled in the Microsoft Defender portal | Portal-only toggle, tenant-wide, shared with every other "Security policy violations…" family policy, §5 Step 3 |
...
Administrator**, or the granular permission [RBAC model §12](/docs/rbac-model/#12-microsoft-defender-for-endpoint-portal-rbac-an-eighth-system-for-scenarios-that-configure-defender-for-endpoint-tenant-wide-settings) documents.
...
. Schedule this script to run on the same recurring cadence as the HR system's own
...
- **Communication Compliance licensing, if that trigger path is used:** Communication Compliance
 ...
 Suite, or an E5 add-on) per [Licensing matrix §2](/docs/licensing-matrix/#2-master-capability--license-matrix), confirm this entitlement is present
