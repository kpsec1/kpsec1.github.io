...
.DESCRIPTION
    ...
    1. AUTOMATED - AUDIT TRAIL (hard pass/fail, contributes to exit code) - read-only, no tenant
       ...
       pci-dss-assessment/validate/Test-ComplianceManagerAuditTrail.ps1, and scenarios/compliance-
       manager/soc2-assessment/validate/Test-ComplianceManagerAuditTrail.ps1, because the CSV shape
       and the operations it monitors are identical - the underlying script is reused, not
       reinvented (see ../design.md Section 2):
    ...
    2. AUTOMATED - CROSSWALK MANIFEST (hard pass/fail, contributes to exit code) - THIS scenario's
       own addition, structured differently from its ISO 27001/PCI DSS/SOC 2 siblings because
       HIPAA/HITECH is organized by its own published rule structure, not a numbered-goal or
       Trust-Services-Criteria shape: structurally validates deploy/policy/hipaa-hitech-assessment-
       manifest.json's controlCrosswalk array (all 5 categories - Privacy Rule, Administrative
       Safeguards, Physical Safeguards, Technical Safeguards, Breach Notification Rule - present
       exactly once, each with a cfrCitation and a coverage string, and each of the three Security
       Rule safeguard categories flagged hasAddressableSpecifications:true so a future edit can't
...
.PARAMETER AuditTrailCsvPath
    ...
    manager/pci-dss-assessment/deploy/, or scenarios/compliance-manager/soc2-assessment/deploy/, if
    that's the one actually run - see design.md Section 2).

.PARAMETER CrosswalkManifestPath
    Path to deploy/policy/hipaa-hitech-assessment-manifest.json. Defaults to the sibling copy in
    this scenario's own deploy/policy/ folder.
...
.PARAMETER RepoRoot
    ...
    location (validate/ -> hipaa-hitech-assessment/ -> compliance-manager/ -> scenarios/ -> repo
    root), matching this repository's own scenario folder depth.
...
.PARAMETER AssessmentName
    ...
    scenario's docs and deploy/policy/hipaa-hitech-assessment-manifest.json.
...
param(
    ...
    [Parameter()]
    [string]$CrosswalkManifestPath = (Join-Path $PSScriptRoot '../deploy/policy/hipaa-hitech-assessment-manifest.json'),

    [Parameter()]
    [string]$RepoRoot = (Join-Path $PSScriptRoot '../../../..'),

    [Parameter()]
    [string]$AssessmentName = 'HIPAA/HITECH - Microsoft 365 Estate'
)
...
if (-not (Test-Path -Path $AuditTrailCsvPath -PathType Leaf)) {
    Write-Host "`nCannot continue audit-trail checks - file not found. Run deploy/Export-ComplianceManagerAuditTrail.ps1 first (or a reused copy in assess-against-iso27001/deploy/, pci-dss-assessment/deploy/, or soc2-assessment/deploy/ - see design.md Section 2)." -ForegroundColor Red
}
else {
...
if (-not (Test-Path -Path $CrosswalkManifestPath -PathType Leaf)) {
    ...
    if ($manifest) {
        Test-Check -Description "regulation is 'HIPAA/HITECH' (not the separately-listed HITRUST catalog entry - README.md Section 11)" -Condition ($manifest.regulation -eq 'HIPAA/HITECH')

        $categories = @($manifest.controlCrosswalk.categories)
        $expectedCategories = 'Privacy Rule', 'Administrative Safeguards', 'Physical Safeguards', 'Technical Safeguards', 'Breach Notification Rule'
        Test-Check -Description "controlCrosswalk.categories contains exactly 5 entries (HIPAA/HITECH's own published rule structure)" -Condition ($categories.Count -eq 5)
        ...
        Test-Check -Description "All 5 categories are present exactly once" -Condition (($missingCategories.Count -eq 0) -and ($categoryNames.Count -eq ($categoryNames | Select-Object -Unique).Count))
        if ($missingCategories.Count -gt 0) {
            Write-Host "         Missing categor(y/ies): $($missingCategories -join ', ')" -ForegroundColor Yellow
        }

        $incompleteCategories = @($categories | Where-Object { -not $_.category -or -not $_.cfrCitation -or -not $_.coverage })
        Test-Check -Description "Every category entry has a category name, a cfrCitation, and a coverage string" -Condition ($incompleteCategories.Count -eq 0)

        $securityRuleCategories = @($categories | Where-Object { $_.category -in @('Administrative Safeguards', 'Physical Safeguards', 'Technical Safeguards') })
        $missingAddressableFlag = @($securityRuleCategories | Where-Object { $_.hasAddressableSpecifications -ne $true })
        Test-Check -Description "All 3 Security Rule safeguard categories are flagged hasAddressableSpecifications:true (addressable is not optional - README.md Section 11)" -Condition ($securityRuleCategories.Count -eq 3 -and $missingAddressableFlag.Count -eq 0)
        if ($missingAddressableFlag.Count -gt 0) {
            Write-Host "         Missing/incorrect flag on: $(($missingAddressableFlag | ForEach-Object { $_.category }) -join ', ')" -ForegroundColor Yellow
        }
...
$manualChecklist = @(
    "Purview portal > Compliance Manager > Assessments: an assessment named '$AssessmentName' exists, based on the 'HIPAA/HITECH' regulation (NOT the separately-listed HITRUST template - README.md Section 11), with status other than blank/None."
    "Assessment's Services tab shows Microsoft 365 in scope, matching deploy/policy/hipaa-hitech-assessment-manifest.json's servicesInScope."
    "Assessment's group matches deploy/policy/hipaa-hitech-assessment-manifest.json's group.name - if assess-against-iso27001, pci-dss-assessment, and/or soc2-assessment are also deployed, confirm this assessment was ADDED to that same group (design.md Section 6), not created in a separate one."
    "Compliance Manager > Settings > User access (or the assessment's own Manage user access pane): role assignments match deploy/policy/hipaa-hitech-assessment-manifest.json's roleAssignments."
    "Confirm the organization has formally DESIGNATED a HIPAA Privacy Officer (45 CFR Section 164.530(a)(1)) and Security Officer (45 CFR Section 164.308(a)(2)) - these are HIPAA-mandated organizational designations, separate from and not satisfied by any Compliance Manager RBAC role assignment."
    "Compliance Manager > Improvement actions, filtered to this assessment: spot-check that at least some actions show a 'Passed'/tested status sourced from built-in automation, confirming already-deployed Information Protection/DLP/Adaptive Protection/Insider Risk/Audit scenarios (deploy/policy/hipaa-hitech-assessment-manifest.json's recommendedDeploymentOrder) are actually feeding this assessment."
    "Spot-check at least one improvement action mapped to an 'addressable' Security Rule implementation specification: confirm its evidence/notes document either (a) implementation as written, or (b) a documented reasonable-alternative measure and rationale - NOT simply marked N/A/skipped, which would misrepresent an addressable specification as optional (README.md Section 11)."
    "If the same nontechnical improvement action (e.g. a documented information security policy) appears in both this assessment and the ISO 27001, PCI DSS, and/or SOC 2 assessments: confirm updating it in one is reflected in the others (design.md Section 6's group-sharing claim) - this is the concrete, checkable proof the group-placement decision is paying off."
    "Confirm with whoever owns this tenant's HIPAA compliance program that this assessment is being used as an internal readiness-tracking tool, and NOT presented anywhere as 'HIPAA certified' or as a substitute for the organization's own required Security Risk Analysis or Business Associate Agreement - README.md Section 2/Section 11."
    "Given HIPAA's own 6-year required retention period for Security Rule documentation (45 CFR Section 164.316(b)(2)(i)): confirm the audit-trail CSV (or an archived copy of it) is retained on a schedule that outlives Audit (Standard)'s 180-day default retention by years, not days - see deploy/Export-ComplianceManagerAuditTrail.ps1's .NOTES."
