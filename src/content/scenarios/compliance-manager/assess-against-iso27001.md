...
fullTitle: "Compliance Manager — Assess Against ISO/IEC 27001:2022"
...
| Automation identity (audit-trail script only) | App registration or account holding the **View-Only Audit Logs** (or **Audit Logs**) Exchange Online role, in addition to `Exchange.ManageAsApp` | `Search-UnifiedAuditLog` requires an **Exchange Online** RBAC role — a Purview-only role (even Compliance Administrator) is explicitly documented as insufficient. See `docs/rbac-model.md` §6 and `docs/automation-surface.md` §3 |
...
1. Before starting, decide the group name and role assignments — see
   ...
   **Both decisions are effectively permanent**: an assessment's group can't be changed after
   creation, and groups themselves can't be deleted [[2]](#references).
2. **(Recommended order, not required)** Deploy this library's DLP, Information Protection, and
   ...
   Compliance, and Insider Risk Management [[6]](#references), so controls already live in the
...
10. **Improvement actions** tab of the assessment: for actions with **Automatic** testing type,
    ...
    before doing any manual work on them [[6]](#references).
...
surface 1 per `docs/automation-surface.md` §1, because Compliance Manager has no surface of its
...
4. **Evidence for an internal or external auditor** — the assessment's own **Export actions**
   ...
   [[7]](#references); this scenario's audit-trail CSV is a **secondary**, complementary artifact
...
- **No additional cost for the audit-trail script** beyond the Exchange Online role assignment —
  ...
  most Microsoft 365 organizations [[11]](#references).
- **Sizing note:** the premium-template allotment is per-regulation, not per-assessment — creating
  ...
  slot [[10]](#references). Plan the grouping strategy (§5, step 1) with this in mind before
...
- **A high compliance score is not proof of certification-readiness.** Microsoft's own FAQ states
  ...
  guarantee [[13]](#references) — do not let this scenario's tooling imply otherwise to a board or
  customer.
