...
.DESCRIPTION
    ...
    1. AUTOMATED - AUDIT TRAIL (hard pass/fail, contributes to exit code) - read-only, no tenant
       ...
       iso27001/validate/Test-ComplianceManagerAuditTrail.ps1, scenarios/compliance-manager/
       pci-dss-assessment/validate/Test-ComplianceManagerAuditTrail.ps1, scenarios/compliance-
       manager/soc2-assessment/validate/Test-ComplianceManagerAuditTrail.ps1, and scenarios/
       compliance-manager/hipaa-hitech-assessment/validate/Test-ComplianceManagerAuditTrail.ps1,
       because the CSV shape and the operations it monitors are identical - the underlying script
       is reused, not reinvented (see ../design.md Section 2):
    ...
    2. AUTOMATED - CROSSWALK MANIFEST (hard pass/fail, contributes to exit code) - THIS scenario's
       own addition, structured differently from its ISO 27001/PCI DSS/SOC 2/HIPAA-HITECH siblings
       because GDPR is organized by its own published structure (Microsoft's gdpr overview groups
       it into Data Subject Requests, Breach Notification, and Data Protection Impact Assessment,
       alongside the Article 5 processing principles, Article 46 cross-border-transfer safeguards,
       and Article 30/37 accountability-and-governance obligations that sit around them) - not a
       numbered-goal shape (PCI DSS), a Trust-Services-Criteria shape (SOC 2), Annex A domains
       (ISO 27001), or a rule-with-safeguard-categories shape (HIPAA/HITECH): structurally validates
       deploy/policy/gdpr-assessment-manifest.json's controlCrosswalk array (all 6 categories -
       Data Subject Rights, Data Processing Principles, Breach Notification, Data Protection Impact
       Assessment, Cross-Border Data Transfers, Accountability & Governance - present exactly once,
       each with an articleCitation and a coverage string) and confirms every scenario path
       referenced anywhere in the manifest (recommendedDeploymentOrder and controlCrosswalk.coverage)
       that looks like a scenarios/... path actually exists in this repository - catching a stale
...
.PARAMETER AuditTrailCsvPath
    ...
    copies in scenarios/compliance-manager/assess-against-iso27001/deploy/, scenarios/compliance-
    manager/pci-dss-assessment/deploy/, scenarios/compliance-manager/soc2-assessment/deploy/, or
    scenarios/compliance-manager/hipaa-hitech-assessment/deploy/, if that's the one actually run -
    see design.md Section 2).

.PARAMETER CrosswalkManifestPath
    Path to deploy/policy/gdpr-assessment-manifest.json. Defaults to the sibling copy in this
...
.PARAMETER RepoRoot
    ...
    location (validate/ -> gdpr-assessment/ -> compliance-manager/ -> scenarios/ -> repo root),
...
.PARAMETER AssessmentName
    ...
    scenario's docs and deploy/policy/gdpr-assessment-manifest.json.
...
param(
    ...
    [Parameter()]
    [string]$CrosswalkManifestPath = (Join-Path $PSScriptRoot '../deploy/policy/gdpr-assessment-manifest.json'),

    [Parameter()]
    [string]$RepoRoot = (Join-Path $PSScriptRoot '../../../..'),

    [Parameter()]
    [string]$AssessmentName = 'EU GDPR - Microsoft 365 Estate'
)
...
if (-not (Test-Path -Path $AuditTrailCsvPath -PathType Leaf)) {
    Write-Host "`nCannot continue audit-trail checks - file not found. Run deploy/Export-ComplianceManagerAuditTrail.ps1 first (or a reused copy in assess-against-iso27001/deploy/, pci-dss-assessment/deploy/, soc2-assessment/deploy/, or hipaa-hitech-assessment/deploy/ - see design.md Section 2)." -ForegroundColor Red
}
else {
...
if (-not (Test-Path -Path $CrosswalkManifestPath -PathType Leaf)) {
    ...
    if ($manifest) {
        Test-Check -Description "regulation is 'EU GDPR (General Data Protection Regulation)' (not the separately-listed UK Data Protection Act catalog entry - README.md Section 11)" -Condition ($manifest.regulation -eq 'EU GDPR (General Data Protection Regulation)')

        $categories = @($manifest.controlCrosswalk.categories)
        $expectedCategories = 'Data Subject Rights (DSR)', 'Data Processing Principles', 'Breach Notification', 'Data Protection Impact Assessment (DPIA)', 'Cross-Border Data Transfers', 'Accountability & Governance'
        Test-Check -Description "controlCrosswalk.categories contains exactly 6 entries (this library's own correlation to GDPR's own published structure)" -Condition ($categories.Count -eq 6)
        ...
        Test-Check -Description "All 6 categories are present exactly once" -Condition (($missingCategories.Count -eq 0) -and ($categoryNames.Count -eq ($categoryNames | Select-Object -Unique).Count))
        if ($missingCategories.Count -gt 0) {
            Write-Host "         Missing categor(y/ies): $($missingCategories -join ', ')" -ForegroundColor Yellow
        }

        $incompleteCategories = @($categories | Where-Object { -not $_.category -or -not $_.articleCitation -or -not $_.coverage })
        Test-Check -Description "Every category entry has a category name, an articleCitation, and a coverage string" -Condition ($incompleteCategories.Count -eq 0)
...
$manualChecklist = @(
    "Purview portal > Compliance Manager > Assessments: an assessment named '$AssessmentName' exists, based on the 'EU GDPR (General Data Protection Regulation)' regulation (NOT the separately-listed 'UK Data Protection Act' catalog entry - README.md Section 11), with status other than blank/None."
    "Assessment's Services tab shows Microsoft 365 in scope, matching deploy/policy/gdpr-assessment-manifest.json's servicesInScope."
    "Assessment's group matches deploy/policy/gdpr-assessment-manifest.json's group.name - if assess-against-iso27001, pci-dss-assessment, soc2-assessment, and/or hipaa-hitech-assessment are also deployed, confirm this assessment was ADDED to that same group (design.md Section 6), not created in a separate one."
    "Compliance Manager > Settings > User access (or the assessment's own Manage user access pane): role assignments match deploy/policy/gdpr-assessment-manifest.json's roleAssignments."
    "Confirm whether Article 37 requires this organization to designate a Data Protection Officer (large-scale systematic monitoring, large-scale special-category data processing, or public-authority processing) - unlike HIPAA's blanket officer requirement, this is CONDITIONAL, and the answer should be documented rather than assumed either way."
    "Compliance Manager > Improvement actions, filtered to this assessment: spot-check that at least some actions show a 'Passed'/tested status sourced from built-in automation, confirming already-deployed Data Map/Information Protection/DLP/Data Lifecycle Management/Audit scenarios (deploy/policy/gdpr-assessment-manifest.json's recommendedDeploymentOrder) are actually feeding this assessment."
    "Confirm the Regulation licenses used counter on the Regulations page reflects this GDPR template counting against the tenant's 3-free-premium-template allotment (not a legacy pre-December-2022 free entitlement that no longer applies) - README.md Section 3/Section 10."
    "If the same nontechnical improvement action (e.g. a documented information security policy) appears in both this assessment and the ISO 27001, PCI DSS, SOC 2, and/or HIPAA/HITECH assessments: confirm updating it in one is reflected in the others (design.md Section 6's group-sharing claim) - this is the concrete, checkable proof the group-placement decision is paying off."
    "Confirm with whoever owns this tenant's GDPR compliance program that this assessment is being used as an internal readiness-tracking tool, and NOT presented anywhere as 'GDPR certified' or as a substitute for the organization's own required Article 30 Records of Processing Activities, Article 35 DPIAs, or Article 28 processor-agreement review - README.md Section 2/Section 11."
    "Confirm the organization has a documented record-retention schedule for this assessment's evidence and the audit-trail CSV consistent with its own Article 5(1)(e) storage-limitation policy - GDPR does not prescribe a fixed number of years the way HIPAA's 45 CFR Section 164.316(b)(2)(i) does, so there is no single correct answer to check against; confirm one exists and is followed."
