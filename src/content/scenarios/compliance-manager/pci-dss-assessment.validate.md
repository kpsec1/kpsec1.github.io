...
.DESCRIPTION
    ...
    1. AUTOMATED - AUDIT TRAIL (hard pass/fail, contributes to exit code) - read-only, no tenant
       ...
       iso27001/validate/Test-ComplianceManagerAuditTrail.ps1, because the CSV shape and the
       operations it monitors are identical - the underlying script is reused, not reinvented (see
       ../design.md Section 2):
    ...
    2. AUTOMATED - CROSSWALK MANIFEST (hard pass/fail, contributes to exit code) - THIS scenario's
       own addition, not present in the ISO 27001 scenario's validate script: structurally
       validates deploy/policy/pci-dss-assessment-manifest.json's controlCrosswalk array (each PCI
       DSS goal 1-6 present exactly once, in range, with a title and a coverage string) and confirms
       every scenario path referenced anywhere in the manifest (recommendedDeploymentOrder and
       controlCrosswalk.coverage) that looks like a scenarios/... path actually exists in this
...
.PARAMETER AuditTrailCsvPath
    Path to the CSV produced by deploy/Export-ComplianceManagerAuditTrail.ps1 (or the reused copy
    in scenarios/compliance-manager/assess-against-iso27001/deploy/, if that's the one actually run
    - see design.md Section 2).

.PARAMETER CrosswalkManifestPath
    Path to deploy/policy/pci-dss-assessment-manifest.json. Defaults to the sibling copy in this
...
.PARAMETER RepoRoot
    ...
    location (validate/ -> pci-dss-assessment/ -> compliance-manager/ -> scenarios/ -> repo root),
...
.PARAMETER AssessmentName
    ...
    scenario's docs and deploy/policy/pci-dss-assessment-manifest.json.
...
param(
    ...
    [Parameter()]
    [string]$CrosswalkManifestPath = (Join-Path $PSScriptRoot '../deploy/policy/pci-dss-assessment-manifest.json'),

    [Parameter()]
    [string]$RepoRoot = (Join-Path $PSScriptRoot '../../../..'),

    [Parameter()]
    [string]$AssessmentName = 'PCI DSS v4.0 - Microsoft 365 Estate'
)
...
if (-not (Test-Path -Path $AuditTrailCsvPath -PathType Leaf)) {
    Write-Host "`nCannot continue audit-trail checks - file not found. Run deploy/Export-ComplianceManagerAuditTrail.ps1 first (or the reused copy in assess-against-iso27001/deploy/ - see design.md Section 2)." -ForegroundColor Red
}
else {
...
if (-not (Test-Path -Path $CrosswalkManifestPath -PathType Leaf)) {
    ...
    if ($manifest) {
        Test-Check -Description "regulation is 'PCI DSS v4.0' (not the retired v3.2.1 template - README.md Section 11)" -Condition ($manifest.regulation -eq 'PCI DSS v4.0')

        $goals = @($manifest.controlCrosswalk.goals)
        Test-Check -Description "controlCrosswalk.goals contains exactly 6 entries (PCI DSS v4.0's 6 control goals)" -Condition ($goals.Count -eq 6)

        $goalNumbers = @($goals | ForEach-Object { $_.goal })
        $expectedGoalNumbers = 1..6
        $missingGoals = @($expectedGoalNumbers | Where-Object { $_ -notin $goalNumbers })
        Test-Check -Description "Goals 1 through 6 are each present exactly once" -Condition (($missingGoals.Count -eq 0) -and ($goalNumbers.Count -eq ($goalNumbers | Select-Object -Unique).Count))
        if ($missingGoals.Count -gt 0) {
            Write-Host "         Missing goal number(s): $($missingGoals -join ', ')" -ForegroundColor Yellow
        }

        $incompleteGoals = @($goals | Where-Object { -not $_.title -or -not $_.requirements -or -not $_.coverage })
        Test-Check -Description "Every goal entry has a title, requirements range, and coverage string" -Condition ($incompleteGoals.Count -eq 0)
        ...
        $textToScan = @($manifest.recommendedDeploymentOrder) + @($goals | ForEach-Object { $_.coverage })
...
$manualChecklist = @(
    "Purview portal > Compliance Manager > Assessments: an assessment named '$AssessmentName' exists, based on the PCI DSS v4.0 regulation (NOT the separately-listed PCI DSS v3.2.1 template - README.md Section 11), with status other than blank/None."
    "Assessment's Services tab shows Microsoft 365 in scope, matching deploy/policy/pci-dss-assessment-manifest.json's servicesInScope."
    "Assessment's group matches deploy/policy/pci-dss-assessment-manifest.json's group.name - if scenarios/compliance-manager/assess-against-iso27001/ is also deployed, confirm this assessment was ADDED to that same group (design.md Section 6), not created in a separate one."
    "Compliance Manager > Settings > User access (or the assessment's own Manage user access pane): role assignments match deploy/policy/pci-dss-assessment-manifest.json's roleAssignments."
    "Compliance Manager > Improvement actions, filtered to this assessment: spot-check that at least some actions show a 'Passed'/tested status sourced from built-in automation, confirming already-deployed DLP/Information Protection/Endpoint DLP/Adaptive Protection/IRM/Audit scenarios (deploy/policy/pci-dss-assessment-manifest.json's recommendedDeploymentOrder) are actually feeding this assessment."
    "If the same nontechnical improvement action (e.g. a documented information security policy) appears in both this assessment and the ISO 27001 assessment: confirm updating it in one is reflected in the other (design.md Section 6's group-sharing claim) - this is the concrete, checkable proof the group-placement decision is paying off."
    "Confirm with whoever owns this tenant's PCI DSS validation program that this assessment is being used as an internal tracking/evidence tool, and NOT presented anywhere as a substitute for a PCI DSS Self-Assessment Questionnaire (SAQ) or a QSA-conducted Report on Compliance (RoC) - README.md Section 2/Section 11."
