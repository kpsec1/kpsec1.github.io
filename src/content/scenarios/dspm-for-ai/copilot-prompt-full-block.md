...
fullTitle: "DSPM for AI — Copilot Prompt Full-Response Block"
...
narrower web-grounding restriction [[1]](#references).
...
- **User-facing deterrence, not just detection** — unlike Rule 1 (silent web-grounding restriction,
  ...
  was blocked by an organizational policy [[1]](#references), which can support a "the control is
...
| "Block sensitive information types in prompts" feature rollout | **Preview**, rolling out to all tenants with Microsoft 365 Copilot/Copilot Chat access as of this writing | Confirm the feature has reached your tenant before relying on this scenario — Microsoft's own guidance says to check rollout status, it is not universally available on a fixed date [[1]](#references). Available in Microsoft 365 Copilot, Copilot Chat, and Copilot in Word/Excel/PowerPoint; during preview, the in-app messaging in Word/Excel/PowerPoint may not clearly say the block is policy-driven [[1]](#references). |
| DLP to author the Copilot-location policy | Same Copilot-location-specific role list as the parent scenario (Purview Data Security AI Admin(s), Compliance Administrator, etc.) | See parent scenario's `README.md` §3 and `docs/rbac-model.md` §3 — unchanged by this addition. |
...
> `docs/licensing-matrix.md` and current Microsoft Learn before a sales commitment — this is a
...
as the parent scenario applies [[1]](#references).
...
5. Action: **Restrict Copilot from processing content** → **Processing prompts** [[1]](#references).
   ...
   [[2]](#references) for this same action — an inconsistency in Microsoft's own wording, not a typo
...
| What it stops | Copilot from responding to the prompt at all — not used for internal or web grounding [[1]](#references) |
...
2. **Functional test** — from a test account, submit a Copilot prompt containing a documented test
   ...
   has blocked Copilot from using [[1]](#references) — contrast with the parent scenario's Rule 1
...
- **Confirm preview-to-GA licensing has not changed** before quoting — Microsoft's licensing
  ...
  specific action separately from the other two; re-verify against `docs/licensing-matrix.md` before
...
- **During preview, Word/Excel/PowerPoint messaging may not clearly attribute the block to policy**
  [[1]](#references) — set user expectations accordingly if this rule is deployed before the feature
  reaches GA.
