...
whoFor: "a data governance or platform team that has already run"
...
| Role to create/edit a critical data element and add columns | **Data Steward AND Data Product Owner**, both, on the governance domain | Microsoft's own prerequisite: "To create critical data elements and add columns to them, you must have data steward and data product owner permissions", a stricter combined requirement than `manage-data-products`' Data Product Owner-only prerequisite. [RBAC model §5](/docs/rbac-model/#5-data-governance-roles-data-map--unified-catalog-a-separate-model) |
...
, even though (unlike data products) there is no separate access-policy gate
to configure first.
