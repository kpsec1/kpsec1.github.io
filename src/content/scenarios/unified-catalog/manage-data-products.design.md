...
. Microsoft's own "Master data management in Microsoft Purview"
...
, which this scenario automates end to end, reusing the domain and
...
GUID, an exact-match server-side filter, not a name-based
...
`end-to-end-lineage-validation` scenarios call directly. A Unified
...
`DataAssetGeneral` object with its **own**, separate GUID. **Data
...
data product"**. A **data product access policy** (who can request
access, whether manager/privacy approval is required, the terms-of-use attestation) is configured
entirely in the portal's **Manage policies** flow, this build's
