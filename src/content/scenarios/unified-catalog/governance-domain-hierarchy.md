...
whoFor: "a data governance team past the single-domain pilot stage, federating governance"
...
- **CDMC ownership/accountability controls**, business-concept attributes (§6 of `design.md`) are
 ...
 (same source `curate-business-glossary/README.md` §2 cites for this claim).
...
| Role to create new domains | **Governance Domain Creator** (catalog-level role) | [RBAC model §5](/docs/rbac-model/#5-data-governance-roles-data-map--unified-catalog-a-separate-model) |
| Role to edit/re-run against existing domains | **Governance Domain Owner** (governance-domain-level role; assigned automatically to whoever creates the domain, [RBAC model §5](/docs/rbac-model/#5-data-governance-roles-data-map--unified-catalog-a-separate-model)) | Required to edit a domain (name, attributes, data estate mapping, status) per Microsoft's own guidance, distinct from the catalog-level **Governance Domain Creator** role needed only to make a *new* domain |
...
| Automation identity | App registration whose service principal is assigned the roles above | Client-credentials OAuth2 against resource `https://purview.azure.net`, [Automation surface §3](/docs/automation-surface/#3-authentication-patterns-interactive-vs-unattended) |
...
- **Data estate mapping is recommended guidance, not an access-control boundary.** Microsoft states
 ...
 ([RBAC model §5](/docs/rbac-model/#5-data-governance-roles-data-map--unified-catalog-a-separate-model)), not by this scenario's mapping. Do not present a data estate mapping to
