...
created in one pass [[1]](README.md#12-references). That path is real and useful for a **one-time**
...
> "The bulk import process **can't be used to edit or update** glossary terms." [[1]](README.md#12-references)
...
blocked [[1]](README.md#12-references). That makes CSV import a one-shot import tool, not a
...
duplicate name, it doesn't block it [[2]](README.md#12-references), and the `Create` operation
requires the caller to supply a client-generated `id` (a GUID), there is no server-assigned,
name-derived identity to key off of [[3]](README.md#12-references). A naive idempotent-upsert
...
(case-insensitive) name match** within that domain [[4]](README.md#12-references). If an exact
...
purpose is "retrieves the count of duplicate Term by passing namekeyword" [[5]](README.md#12-references)
...
person or group" [[6]](README.md#12-references), a raw Microsoft Entra object ID, not an email
address. A human-authored glossary definition file naturally lists owners by UPN/email (as the
portal's own CSV import does [[1]](README.md#12-references)), so this script resolves each
`owners`/`experts` entry that isn't already a GUID to its Entra object ID via a single Microsoft
Graph `GET /v1.0/users/{upn}?$select=id` call per identity [[7]](README.md#12-references), using
the same app registration with the least-privileged `User.Read.All` **application** permission
[[8]](README.md#12-references). This is the one place this scenario's automation spans two
...
setup walkthrough [[10]](README.md#12-references).
...
- **Linking terms to data products, data assets, or columns.** That requires the target assets to
  ...
  Steward on the domain [[1]](README.md#12-references), a natural follow-up scenario once a Data
