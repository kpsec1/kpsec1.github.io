...
.DESCRIPTION
    Read-only validation script - never modifies any object. Checks:
      1. The scan rule set exists, is kind AzureSqlDatabaseManagedInstance and scanRulesetType
         Custom.
...
.PARAMETER ScanRulesetName
    Name of the custom scan rule set to validate. Defaults to
...
Test-Check -Description "Scan rule set kind is AzureSqlDatabaseManagedInstance" -Condition ($ruleset.kind -eq 'AzureSqlDatabaseManagedInstance')
...
if ($scan) {
    ...
    Test-Check -Description "Scan kind is a recognized Azure SQL Managed Instance kind ($($scan.kind))" `
        -Condition ($scan.kind -in @('AzureSqlDatabaseManagedInstanceMsi', 'AzureSqlDatabaseManagedInstanceCredential'))
}
