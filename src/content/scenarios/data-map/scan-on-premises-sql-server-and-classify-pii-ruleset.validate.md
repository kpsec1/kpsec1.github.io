...
.DESCRIPTION
    Read-only validation script - never modifies any object. Checks:
      1. The scan rule set exists, is kind SqlServerDatabase and scanRulesetType Custom.
      ...
      4. The target scan exists and its scanRulesetName/scanRulesetType point at this ruleset, and
         its kind is the one compatible on-premises SQL Server scan kind.
    Exits with a non-zero code if any hard check fails. Safe to re-run any number of times.

    Does NOT check self-hosted integration runtime node health (Running/Disconnected/Offline) -
    the Integration Runtimes - Get REST operation returns the resource definition, not live node
    status, so a scan can pass every check here and still fail at run time if no SHIR node has
    registered - see the base scenario's README.md Section 7 check 2, which this script does not
    duplicate.
...
.PARAMETER ScanRulesetName
    Name of the custom scan rule set to validate. Defaults to 'SqlServerDatabase-PiiOnly' to match
...
.NOTES
    Sources: same references as deploy/New-PiiOnlyScanRuleset.ps1's .NOTES.
#>
[CmdletBinding()]
param(
    [Parameter(Mandatory)]
...
Test-Check -Description "Scan rule set kind is SqlServerDatabase" -Condition ($ruleset.kind -eq 'SqlServerDatabase')
...
if ($scan) {
    ...
    Test-Check -Description "Scan kind is SqlServerDatabaseCredential (the only compatible on-premises SQL Server scan kind)" `
        -Condition ($scan.kind -eq 'SqlServerDatabaseCredential')
    Test-Check -Description "Scan still has a connectedVia self-hosted integration runtime reference" `
        -Condition ($null -ne $scan.properties.connectedVia -and $scan.properties.connectedVia.referenceName)
}

Write-Host "`n$(if ($script:failures -eq 0) { 'All hard checks passed.' } else { "$script:failures hard check(s) failed." })" `
    -ForegroundColor $(if ($script:failures -eq 0) { 'Green' } else { 'Red' })
if ($script:failures -eq 0) {
    Write-Host "Reminder: this script cannot check self-hosted integration runtime node health (Running/Disconnected/Offline) - confirm that separately in the portal (Data Map -> Integration runtimes -> the runtime -> Nodes tab) before relying on a passing result here to predict a scan run's success." -ForegroundColor Yellow
}
