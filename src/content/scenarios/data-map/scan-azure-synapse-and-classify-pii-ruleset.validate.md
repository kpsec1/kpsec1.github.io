...
.DESCRIPTION
    Read-only validation script - never modifies any object. Checks:
      1. The scan rule set exists, is kind AzureSynapseWorkspace and scanRulesetType Custom.
...
.PARAMETER ScanRulesetName
    Name of the custom scan rule set to validate. Defaults to 'AzureSynapseWorkspace-PiiOnly' to
    match New-PiiOnlyScanRuleset.ps1's default.
...
Test-Check -Description "Scan rule set kind is AzureSynapseWorkspace" -Condition ($ruleset.kind -eq 'AzureSynapseWorkspace')
...
if ($scan) {
    ...
    Test-Check -Description "Scan kind is a recognized Azure Synapse Analytics kind ($($scan.kind))" `
        -Condition ($scan.kind -in @('AzureSynapseWorkspaceMsi', 'AzureSynapseWorkspaceCredential'))
}
