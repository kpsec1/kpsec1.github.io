...
#### `Test-AzureSynapseDataMapScan.ps1`

```powershell
#Requires -Version 7.0
<#
.SYNOPSIS
    Verifies the Azure Synapse Analytics workspace data source and scan created by
    New-AzureSynapseDataMapScan.ps1 are configured as expected, and reports the most recent scan
    run's status.

.DESCRIPTION
    Read-only validation script - never modifies any object. Checks:
      1. The data source exists with kind AzureSynapseWorkspace and at least one of the expected
         dedicated/serverless SQL endpoints.
      2. The scan exists with kind AzureSynapseWorkspaceMsi and the expected scan rule set.
...
.PARAMETER ExpectedDedicatedSqlEndpoint
    Optional. If supplied, checked against the registered data source's dedicated SQL endpoint.

.PARAMETER ExpectedServerlessSqlEndpoint
    Optional. If supplied, checked against the registered data source's serverless SQL endpoint.

.PARAMETER ApiVersion
    Data Map REST API version to pin. Defaults to '2023-09-01' to match the deploy script.

.EXAMPLE
    ./Test-AzureSynapseDataMapScan.ps1 -PurviewAccountName 'contoso-purview' -TenantId $TenantId `
...
.NOTES
    ...
    same nested shape the scan-azure-sql-managed-instance-and-classify sibling scenario's validate
    script confirmed by direct fetch of Microsoft's own REST reference (generic across data source
    kinds).
#>
[CmdletBinding()]
param(
    ...
    [Parameter()]
    [string]$ExpectedDedicatedSqlEndpoint,

    [Parameter()]
    [string]$ExpectedServerlessSqlEndpoint,
...
# --- Check 1: data source exists and is the right kind/endpoint(s) ---
$dataSource = $null
try {
...
Test-Check -Description "Data source kind is AzureSynapseWorkspace" `
    -Condition ($dataSource.kind -eq 'AzureSynapseWorkspace')

$hasDedicated = -not [string]::IsNullOrEmpty($dataSource.properties.dedicatedSqlEndpoint)
$hasServerless = -not [string]::IsNullOrEmpty($dataSource.properties.serverlessSqlEndpoint)
Test-Check -Description "Data source has at least one SQL endpoint configured (dedicated: $hasDedicated, serverless: $hasServerless)" `
    -Condition ($hasDedicated -or $hasServerless)

if ($ExpectedDedicatedSqlEndpoint) {
    Test-Check -Description "Dedicated SQL endpoint matches '$ExpectedDedicatedSqlEndpoint'" `
        -Condition ($dataSource.properties.dedicatedSqlEndpoint -eq $ExpectedDedicatedSqlEndpoint)
}
if ($ExpectedServerlessSqlEndpoint) {
    Test-Check -Description "Serverless SQL endpoint matches '$ExpectedServerlessSqlEndpoint'" `
        -Condition ($dataSource.properties.serverlessSqlEndpoint -eq $ExpectedServerlessSqlEndpoint)
}
...
if ($scan) {
    Test-Check -Description "Scan kind is AzureSynapseWorkspaceMsi (SAMI-authenticated)" `
        -Condition ($scan.kind -eq 'AzureSynapseWorkspaceMsi') -Warn
    ...
    if ($latestRun) {
        ...
        Test-Check -Description "At least one scan run has completed (none found - run New-AzureSynapseDataMapScan.ps1 with -RunNow, or wait for the recurring trigger)" `
