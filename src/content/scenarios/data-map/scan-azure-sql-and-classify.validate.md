...
#### `Test-AzureSqlDataMapScan.ps1`

```powershell
#Requires -Version 7.0
<#
.SYNOPSIS
    Verifies the Azure SQL Database data source and scan created by New-AzureSqlDataMapScan.ps1
    are configured as expected, and reports the most recent scan run's status.

.DESCRIPTION
    Read-only validation script - never modifies any object. Checks:
      1. The data source exists with kind AzureSqlDatabase and the expected server endpoint.
      2. The scan exists with kind AzureSqlDatabaseMsi and the expected database/scan rule set.
...
Test-Check -Description "Data source kind is AzureSqlDatabase" `
    -Condition ($dataSource.kind -eq 'AzureSqlDatabase')
...
if ($scan) {
    Test-Check -Description "Scan kind is AzureSqlDatabaseMsi (SAMI-authenticated)" `
        -Condition ($scan.kind -eq 'AzureSqlDatabaseMsi') -Warn
    ...
    # --- Check 3: most recent scan run status (VERIFY - see .NOTES) ---
    try {
