...
<#
.SYNOPSIS
    Creates a custom, PII-only Data Map scan rule set for Azure Synapse Analytics (every system
...
.DESCRIPTION
    ...
    Same pattern as the sibling scenarios/data-map/scan-azure-sql-and-classify-pii-ruleset/:
      ...
      3. PUT (create-or-replace) the custom AzureSynapseWorkspaceScanRuleset object - `kind:
         AzureSynapseWorkspace`, confirmed directly against the Az.Purview PowerShell module's own
         GitHub source (reference 1 below; NOT the same string as the System default ruleset's own
         NAME, 'AzureSynapseSQL' - see README.md Section 11's naming-trap callout).
      4. GET the existing scan object, then PUT it back with only `scanRulesetName`/
         ...
         dedicated/serverless endpoints, collection, and the base scenario's own deliberately
         omitted `resourceTypes` field) exactly as the base scenario left it.
...
.PARAMETER DataSourceName
    ...
    (defaults to the workspace name in that base scenario).
...
.PARAMETER ScanRulesetName
    Name for the custom scan rule set object. Defaults to 'AzureSynapseWorkspace-PiiOnly'. Scan
    rule sets are account-wide objects, not scoped to a collection or a single scan (no
    `CollectionReferenceName`-equivalent parameter exists on the
    New-AzPurviewAzureSynapseWorkspaceScanRulesetObject constructor - reference 1) - one PII-only
    ruleset can be reused by every Azure Synapse Analytics scan in the account that wants this same
    narrower scope.
...
.PARAMETER ApiVersion
    Data Map REST API version to pin. Defaults to '2023-09-01', matching the base scenario and the
    Azure SQL Database sibling.
...
.EXAMPLE
    ...
    reconciles the existing scan onto it, and starts an immediate full re-scan of both configured
    pools.

.NOTES
    GROUNDED (this build): the Custom AzureSynapseWorkspaceScanRuleset object shape and its `Kind:
    AzureSynapseWorkspace` value were confirmed via direct fetch (raw.githubusercontent.com) of the
    Az.Purview PowerShell module's own New-AzPurviewAzureSynapseWorkspaceScanRulesetObject.md
    source, after learn.microsoft.com's REST API reference pages returned EGRESS_BLOCKED in this
    ...
    1. New-AzPurviewAzureSynapseWorkspaceScanRulesetObject (Az.Purview PowerShell module - confirms
       the Custom AzureSynapseWorkspaceScanRuleset shape and Kind: AzureSynapseWorkspace, direct
       fetch via GitHub raw source, this build):
    ...
    2. New-AzPurviewAzureSynapseWorkspaceCredentialScanObject (confirms Kind:
       AzureSynapseWorkspaceCredential and the shared 'AzureSynapseSQL' System ruleset name, direct
       fetch via GitHub raw source, this build):
    ...
    5. Connect to and manage Azure Synapse Analytics workspaces in Microsoft Purview (confirms
       'AzureSynapseSQL' as the System default scan rule set name):
...
try {
    ...
    if ($_.Exception.Response -and $_.Exception.Response.StatusCode -eq 404) {
        throw "Scan '$ScanName' on data source '$DataSourceName' was not found. This scenario reconciles an EXISTING scan onto a new ruleset - run scenarios/data-map/scan-azure-synapse-and-classify/deploy/New-AzureSynapseDataMapScan.ps1 first."
    }
    throw
}

# Guard against reconciling a ruleset built for the 'AzureSynapseWorkspace' data source type onto a
# same-named-but-wrong scan (e.g. a -ScanName/-DataSourceName typo colliding with an unrelated
# scan). Same Red Team-flagged guard the Azure SQL Database sibling's script already carries: the
# ruleset's `kind` must match the scan's own source type, and PUTting a mismatched scanRulesetName
# silently succeeds today - it only fails at scan RUN time, far away from the mistake that caused
# it.
...
if ($existingScan.kind -notin $compatibleScanKinds) {
    throw "Scan '$ScanName' has kind '$($existingScan.kind)', not one of $($compatibleScanKinds -join '/'). This scenario's scan rule set is kind 'AzureSynapseWorkspace' and is only compatible with an Azure Synapse Analytics scan - refusing to reconcile a mismatched scan (double-check -DataSourceName/-ScanName)."
}
...
# to -ScanRulesetName 'AzureSynapseWorkspace-PiiOnly' would silently clobber each other's
# retained-list choice via this create-or-replace PUT. Same guard the Azure SQL Database sibling
# uses: warn loudly on a content change to a pre-existing ruleset (a brand-new ruleset, or a re-run
# with identical content, stays silent) so a shared-object overwrite is never invisible to the
...
if ($RunNow) {
    ...
    if ($PSCmdlet.ShouldProcess("Scan '$ScanName'", "Start immediate Full run (runId $runId)")) {
        ...
        Write-Host "Scan run started (runId: $runId). A first full scan of a nontrivial Synapse workspace can take from minutes to hours - poll validate/Test-PiiOnlyScanRuleset.ps1 or the portal for status." -ForegroundColor Cyan
    }
    else {
