...
./deploy/Remove-PiiOnlyScanRuleset.ps1 `
    -PurviewAccountName 'contoso-purview' -TenantId $TenantId -AppId $AppId -ClientSecret $ClientSecret `
    -DataSourceName 'sql-contoso-prod-customerdb' -DeleteRuleset
```
...
if ($scan.properties.scanRulesetType -eq 'System') { Write-Host "Confirmed: scan reverted to System ruleset." -ForegroundColor Green }
...
the ruleset (Stage 2), and the scan's configuration pane should show **System default** as its
rule set (Stage 1).
