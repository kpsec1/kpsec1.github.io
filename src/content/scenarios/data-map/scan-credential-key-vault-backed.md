...
fullTitle: "Data Map — Key Vault-Backed Scan Credential (SQL Auth / Service Principal)"
category: "Data Map"
...
|---|---|---|
| Microsoft Purview account + Data Map | Active **Azure subscription**; Data Map is **PAYG-billed Azure consumption**, not a per-user M365 entitlement | `docs/licensing-matrix.md` §1–2. This scenario creates configuration objects only and adds **no** metered consumption by itself — see §10 |
| Create/replace credential and Key Vault connection objects | **Data Source Administrator** on the target collection (or a parent, by inheritance) | `docs/rbac-model.md` §5. **VERIFY:** Microsoft defines this role as managing "data sources and scans" [[15]](#references) and never names *credentials* in any role description — Data Source Administrator is this repo's by-analogy assumption (credentials are Scanning-plane objects alongside data sources and scans), not a documented requirement. Confirm on a pilot tenant before designing least-privilege around it; if it proves insufficient, Collection Admin is the fallback |
| Assign that role to the automation service principal | **Collection Admin** at the root collection | Only a Collection Admin can grant Data Map data-plane roles — `docs/rbac-model.md` §5, [[7]](#references) |
| Read back the objects (the `validate/` script) | **Data Reader** on the target collection | Least-privilege for the read-only path |
| An **Azure Key Vault**, and the secret inside it | Vault exists; the password / service-principal key is already stored as a secret | Azure-side prerequisite. This scenario never creates a vault or writes a secret — §5 step 1, [[6]](#references). **Use a Key Vault dedicated to Purview scan credentials** — see the next row for why |
| Purview's access to that vault | The Purview account's managed identity needs **Get** + **List** on **secrets** (access-policy model), **or** the **Key Vault Secrets User** role (Azure RBAC permission model) | Grant exactly one, matching the vault's permission model. This is the single most common cause of a credential that validates structurally but fails at scan time — [[6]](#references)[[9]](#references). **Note the blast radius:** both grants are **vault-wide over secrets** — neither can be scoped to individual secrets, so the Purview managed identity gains read access to *every* secret in that vault. Point Purview at a dedicated scan-credential vault rather than a shared application vault |
...
| (ServicePrincipal kind only) the scan's service principal | App registration with a client secret in the vault, plus source-side access (e.g. `db_datareader`) | A **different** principal from the one calling the Purview API — see `design.md` §4, [[10]](#references) |
| Automation identity for the REST calls | App registration with the Purview roles above; client-secret app-only OAuth2 | `docs/automation-surface.md` §3 and surface 4 |
...
2. **Grant Purview access to the vault.** Match your vault's permission model — granting the wrong
   one silently does nothing [[6]](#references):
...
3. **Register the Key Vault connection in Purview.** In the [Microsoft Purview
   ...
   **Create**, and confirm it appears in the list [[6]](#references).
4. **Create the credential.** On the **Credentials** page, select **+ New**. Provide a **Name**,
   ...
   view as ready to use [[6]](#references).
...
|---|---|---|---|
| Key Vault connection | `PUT {endpoint}/scan/azureKeyVaults/{azureKeyVaultName}` | Create-or-replace | [[4]](#references) |
| Credential | `PUT {endpoint}/scan/credentials/{credentialName}` | Create-or-replace | [[1]](#references) |
...
single hyphens; no underscores, no leading/trailing hyphen [[1]](#references)[[4]](#references).
...
`DelegatedAuth`, and `ManagedIdentity` [[1]](#references). This scenario scripts the three that
...
7. `-CheckKeyVaultSecret`: the referenced secret exists and is enabled in the vault, with a warning
   ...
   `-AsPlainText`, so the value is never retrieved [[11]](#references).
...
4. If **everything validates but the scan still fails** — the failure is on one of the two legs
   ...
   - *Purview → Key Vault:* confirm the Purview managed identity still has **Get** + **List** on
     ...
     hasn't blocked it [[6]](#references)[[9]](#references). Verify the secret **name and version**
     are exactly the ones the credential references [[9]](#references).
...
`204 No Content` on success [[3]](#references), and a `404` (already gone) is treated as success so
...
- **This scenario adds no metered consumption.** It creates two small configuration objects.
  ...
  neither of which a credential object touches — see `docs/licensing-matrix.md` §1–2.
...
- **VERIFY (pilot tenant): the two `KeyVaultSecret` discriminator literals.** Microsoft's Purview
  ...
  secret reference appears anywhere in the Purview documentation set** [[1]](#references). The
  ...
  [[8]](#references); and (b) Purview's **own** Key Vault Connections worked *response* returns an
  `id` ending in `/linkedservices/AzureKeyVault1` — Purview does model a Key Vault connection as a
  linked service internally, which makes that store discriminator coherent [[4]](#references).
...
- **VERIFY (pilot tenant): omitted `secretVersion` semantics.** Microsoft's Purview reference
  ...
  documented as defaulting to the latest version [[8]](#references), and Purview's own
  troubleshooting guidance tells you to "use the right secret name **and version**"
  [[9]](#references) without saying whether the version is optional. §8's rotation guidance
...
- **A credential can be silently re-pointed, and Purview does not appear to record it.**
  ...
  [[13]](#references) — and the `Security` diagnostic-log category's own description is scoped to
  "role assignments to a collection or creation or deletion of a collection"
  [[14]](#references). Neither `ScanStatusLogEvent` nor `DataSensitivityLogEvent`, the other two
...
- **Prefer managed identity when you can.** Microsoft's own guidance is to use a managed identity
  ...
  [[12]](#references), and its data-governance security best-practices guidance gives an explicit
