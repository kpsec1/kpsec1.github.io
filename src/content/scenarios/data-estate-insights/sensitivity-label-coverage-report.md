...
fullTitle: "Data Estate Insights — Exportable, Historical Sensitivity-Label Coverage Report"
...
> of this build [[6]](#references)[[7]](#references). Re-check GA status before a production-facing
> deployment — see §11.
...
managing access to specific repositories or files" [[1]](#references). Where the sibling
...
> [[6]](#references)[[7]](#references). This report measures **labeling coverage**, i.e. "is
...
| Run this scenario's report script | **Data Reader** role on the collection(s) in scope | Discovery - Query is a Catalog Data-plane **read** operation [[3]](#references)[[8]](#references) — deliberately narrower than what viewing the native report through "Export to CSV" requires (next row). A credential with this role can already read every label value in scope one asset at a time via the portal; this scenario changes *how conveniently* that data can be aggregated, not the underlying read boundary — see §11's report-sensitivity note before deciding where the output may be stored |
| View **and export** the *native* Data Estate Insights sensitivity-labels report (for comparison, not required by this scenario) | **Insights Reader** role (assignable only by the root collection's Data Curator) *plus* Data Reader on the classic experience — or **Data health reader** on the new Unified Catalog experience [[1]](#references) — and even then, **only a Data Curator can select "Export to CSV"**; a Data Reader + Insights Reader can view but not export [[4]](#references) | This scenario's own script needs none of this — see `design.md` §2 goal 2 |
| Grant the automation identity a Purview role at all | **Collection Admin** role at root (or the relevant sub-collection) to perform the role assignment | Only a Collection Admin can assign Data Reader to a service principal [[3]](#references) |
| Automation identity for the REST calls | App registration with **Data Reader** Purview role on the collection(s) in scope | Client-secret app-only OAuth2, same token endpoint as this repo's other surface-4 scripts — `docs/automation-surface.md` §3 |
...
1. Confirm "Extend sensitivity labels to Data Map" is turned on for the tenant and at least one
   ...
   [[6]](#references)[[7]](#references). This is a manual, licensed, preview-capability prerequisite —
...
3. Note the KPIs shown — number of subscriptions, unique labels applied, sources/files/tables
   ...
   **Export to CSV**. A Data Reader with only the Insights Reader (or Data health reader) role will
   see the report but not the export option [[4]](#references) — this is the specific gap this
...
`docs/automation-surface.md` §1 — the same surface this repo's Data Map, Unified Catalog, Data
...
|---|---|---|
| Object types reported by default | `Tables`, `Files` | Mirrors both the native Classic sensitivity labels report's own files/tables-labeled split and the sibling `classification-coverage-report` scenario's default [[1]](#references) |
...
| Total-count source | The same query's own `@search.count` field | Documented as authoritative — "the total number of search results, not the number of documents in a single page" [[2]](#references) |
| Pagination | `continuationToken`, page size 1000 (the documented maximum) [[2]](#references) | Every page's record count is reconciled against `@search.count`; a mismatch is a warning, not a silent gap — see §11 |
...
- **The native Data Estate Insights application itself carries no separate bill** — same as the
  sibling scenario's own note [[5]](#references); this scenario's report is an *adjunct* to that free
...
- **A high `PercentLabeled` is a labeling-coverage metric, not a protection-coverage metric — do not
  ...
  — DLP is supported only for Microsoft 365 apps and services [[6]](#references)[[7]](#references).
...
- **`0% labeled` for a collection or object type is ambiguous between two structurally different
  ...
  (public preview), Power BI) [[1]](#references) — a source type outside that list will always read
...
- **"Extend sensitivity labels to Data Map" is a Public Preview capability as of this build**
  [[6]](#references)[[7]](#references) — everything this scenario reports on depends on it. Re-check
...
- **No documented Discovery - Query filter for "has any label" / "has no label" exists.** Microsoft's
  ...
  analogous worked example for `label` [[2]](#references) — see `design.md` §2 goal 3. Rather than
