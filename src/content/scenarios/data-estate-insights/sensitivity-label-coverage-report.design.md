...
. It is automatically generated, requires no scripting, and is the right first stop
...
. This scenario's `PercentLabeled` metric is therefore a *labeling-coverage* signal
...
, the direct structural analog of the `classification` field the sibling
...
2. **Use a materially narrower role than viewing the native report requires.** Identical to the
 ...
 operation, no Insights Reader role assignment, and no Data
...
. Rather than invent one, this scenario applies the identical client-side-tally
...
- **The Discovery - Query REST API** is Microsoft's own documented mechanism for retrieving Data Map
 search results and facet aggregations, the same index the Insights application
...
| Facet-only fast path | Offered as `-Mode Facets` (optional, off by default), using the confirmed `label` facet (one of exactly four documented Discovery - Query facets: `assetType`, `classification`, `contactId`, `label`) | Matches the native "Top labels applied across sources/files/tables" charts, but (like those charts) double-counts a multi-labeled asset under every label it carries, and cannot compute an unlabeled count |
...
. Unlike the sibling `classification-coverage-report` scenario
...
- **No official Microsoft-named "Unlabeled assets" KPI exists to reproduce**, unlike the sibling
 ...
 classification on the entity or its columns"). This scenario's own "unlabeled"
