...
fullTitle: "Data Estate Insights — Exportable, Historical Classification Coverage Report"
...
specific classifications and understand required actions" [[2]](#references). This scenario doesn't
...
`docs/automation-surface.md` §1 — the same surface this repo's Data Map, Unified Catalog, and Data
...
|---|---|---|
| Object types reported by default | `Tables`, `Files` | Mirrors the native Classic classifications report's own classified-files/classified-tables split [[2]](#references). Other confirmed valid values: `Folders`, `Glossary terms`, `Dashboards`, `Data pipelines`, `Reports`, `Stored procedures` [[5]](#references) |
| Classified/unclassified computation | Client-side tally of each `SearchResultValue.classification[]` array (empty vs. non-empty), from paginated `-Mode Full` results | Matches the native "Unclassified assets" KPI's own definition — "Assets with no system or custom classification on the entity or its columns" [[3]](#references) — computed from a different, scriptable primitive. No documented Discovery - Query request filter exists for "has any/no classification" — see §11 |
| Total-count source | The same query's own `@search.count` field | Documented as authoritative — "the total number of search results, not the number of documents in a single page" [[5]](#references) |
| Pagination | `continuationToken`, page size 1000 (the documented maximum) [[5]](#references) | Every page's record count is reconciled against `@search.count`; a mismatch is a warning, not a silent gap — see §11 |
...
3. **Cross-check against the native report** — open the same collection's **Classic classifications**
   ...
   (the native report's own refresh cadence is weekly by default [[4]](#references); this scenario's
...
- **The native Data Estate Insights application itself carries no separate bill** — Microsoft states
  ...
  enabled [[4]](#references); this scenario's report is an *adjunct* to that free capability, not a
...
- **No documented Discovery - Query filter for "has any classification" / "has no classification"
  ...
  [[5]](#references), and a Microsoft Q&A thread on exactly this question confirms no broader filter
  is exposed [[8]](#references). Rather than approximate one with an unconfirmed expression (e.g. a
...
- **This scenario reports on classifications only — not sensitivity labels or glossary/curation
  ...
  [[1]](#references). Both are natural sibling fragments (the `label` field on the same
