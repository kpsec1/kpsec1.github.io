...
| Role to author/edit DLP policies | **DLP Compliance Management** role (built into the *Compliance Administrator* / custom S&C role group) | See [RBAC model §3](/docs/rbac-model/#3-microsoft-entra-roles-that-map-into-purview) (Purview role groups) |
| Automation identity | App registration with **Exchange Online Protection → `Exchange.ManageAsApp`** application permission, granted the DLP-authoring role group | Certificate-based app-only auth, see [Automation surface §3](/docs/automation-surface/#3-authentication-patterns-interactive-vs-unattended) |
...
4. **Assign admin units**: accept **Full directory** (unless the tenant uses administrative
 units, see [RBAC model §4](/docs/rbac-model/#4-purview-role-groups-by-module-representative-not-exhaustive)).
...
`New-DlpComplianceRule`), automation surface 2 per [Automation surface §1](/docs/automation-surface/#1-five-automation-surfaces-not-one-read-this-first), because DLP
...
living only in the Purview portal, see [Automation surface §4](/docs/automation-surface/#4-routing-table-which-surface-for-which-purview-task) for the Audit Search /
...
against DLP alert data (per-user override count, [Automation surface §4](/docs/automation-surface/#4-routing-table-which-surface-for-which-purview-task) audit-search
...
- **No PAYG component.** Teams chat/channel DLP is a per-user entitlement feature, not billed
 through Purview's Azure consumption model, see [Licensing matrix §1](/docs/licensing-matrix/#1-the-two-billing-models-read-this-first), 2. Cost is the
...
. If a buyer's process assumes an email trail for every block, this scenario's
...
, and `BlockAccess` is the same underlying Security & Compliance PowerShell
