...
fullTitle: "DLP — Exchange PII Exfiltration Block (Block or Encrypt)"
category: "DLP"
categorySlug: "dlp"
...
|---|---|---|
| DLP for Exchange Online | **Microsoft 365 E3** (basic) | This scenario uses only built-in sensitive information types and standard conditions — no advanced classification, no Teams — so it does not require the E5-tier uplift `docs/licensing-matrix.md` §DLP row notes for advanced classification/Teams. |
| `-Action Encrypt` mode (Microsoft Purview Message Encryption / `EncryptRMSTemplate`) | Included in the same **Office 365 / Microsoft 365 Enterprise E3 or E5** entitlement — no extra license for the basic Encrypt-Only/Do Not Forward templates this scenario uses [[7]](#references)[[8]](#references) | Only the **Advanced** Message Encryption add-on (expiration, revocation, custom branding) needs a higher tier — not used by this scenario. |
| Azure Rights Management activated | Usually active by default for eligible plans | Prerequisite for `-Action Encrypt` only. Confirm with `Get-IRMConfiguration` (`AzureRMSLicensingEnabled = $true`); see `design.md` §7 and Microsoft's "Set up Message Encryption" guide [[9]](#references). |
| Role to author/edit the DLP policy and rules | **Compliance Administrator**, **Compliance Data Administrator**, or a custom role group with the **DLP Compliance Management** role | `docs/rbac-model.md` §3, DLP row. |
...
5. Rule `PII-Exchange-Protect-External`: **Conditions** → **Content contains** → **Sensitive info
   ...
   **1** each, **Any of these** (OR). Add condition **Recipient is** → **outside my organization**
   (`AccessScope: NotInOrganization`) [[3]](#references). **Actions** → **Restrict access or
...
`New-DlpComplianceRule`) — automation surface 2 per `docs/automation-surface.md` §1, the same
...
other DLP scenario in this library; see `docs/automation-surface.md` §4 for building a custom
...
- **No PAYG component for M365 mail.** Base DLP for Exchange is included at **E3**, not E5 — this
  ...
  tier, per `docs/licensing-matrix.md` §DLP row. This is a genuine cost advantage over
...
- **`-Action Encrypt` adds no incremental license cost** for the basic templates this scenario
  ...
  Encryption [[7]](#references)[[8]](#references). Only Advanced Message Encryption (not used
...
- **Bifurcation means "one message" is not always "one decision."** A message to a mixed
  ...
  evaluate it, each generating its own alert/incident report [[6]](#references). An operator
...
- **`-Action Encrypt`'s default RMS template name is not independently confirmed for every
  ...
  once Message Encryption is active [[12]](#references), but neither that documentation nor the
