...
whoFor: "any buyer who has deployed (or is deploying)"
...
| Role to author/edit the DLP rule | **Compliance Administrator**, **Compliance Data Administrator**, or a custom role group with the **DLP Compliance Management** role | [RBAC model §3](/docs/rbac-model/#3-microsoft-entra-roles-that-map-into-purview), DLP row, identical to the parent scenario. |
...
4. Leave **Priority** unset if this is the first rule created after the parent policy's existing
 ...
 policy. If reconciling via script, see §6 for the explicit `-Priority` this
...
[Automation surface §1](/docs/automation-surface/#1-five-automation-surfaces-not-one-read-this-first). It does not call `New-DlpCompliancePolicy`, this fragment never
