...
. This build independently confirmed the exact `-EndpointDlpRestrictions`
 ...
 `Set-DlpComplianceRule`'s official reference pages. The
...
. This scenario uses the former: `-ContentFileTypeMatches` (the parameter
...
Protection, one for Teams/Exchange, one for Devices, not one combined
...
| DLP policy + rules: clipboard/USB/network-share/print restrictions | `deploy/New-AdaptiveProtectionDevicesDlpPolicy.ps1` → `New-/Set-DlpCompliancePolicy`, `New-/Set-DlpComplianceRule -SharedByIRMUserRisk -EndpointDlpRestrictions` | **Yes**, independently grounded |
...
Protection applies here identically.

## 6. Key decisions

| Decision | Choice | Rationale |
|---|---|---|
| Settings scripted | `RemovableMedia`, `CopyPaste`, `NetworkShare`, `Print` only | The only four `-EndpointDlpRestrictions` Setting names with a confirmed, documented Block/Audit/Ignore/Warn action shape, matches four of the six activities in Microsoft's own Devices Quick Setup rule table. |
...
| Two separate policies (this scenario + the Exchange/Teams sibling) | Not merged into one | Matches Microsoft's own two-policy Quick Setup output exactly, see §3. |
...
- **This scenario does not script the tenant-wide restricted-apps/browsers/domains list
 ...
 worked examples), even though that mechanism is real and scriptable. Two
