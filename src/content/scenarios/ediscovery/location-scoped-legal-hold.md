...
fullTitle: "eDiscovery — Location-Scoped Legal Hold (Regulatory Sweep / Shared Mailbox)"
category: "eDiscovery"
...
(`docs/automation-surface.md` §3). Unlike the custodian scenario, **no v1.0 typed PowerShell
...
1. **Open (or create) the case**: Purview portal → **eDiscovery** → **Cases** → select the case
   (or **Create case** if this is a standalone matter) [[1]](#references).
2. **Create the hold policy**: in the case, **Hold policies** tab → **Create hold policy** → name
   ...
   query to scope what's held → **Apply hold** [[2]](#references).
...
`notApplied`, `applied`, `applying`, `removing`, `partial` [[4]](#references). Policy-level
`policyStatus` values: `Pending`, `Error`, `Success` [[5]](#references).
...
3. **Functional proof (portal)** — open the case's **Hold policies** tab and confirm the policy
   shows **On** with every location's status green/no errors [[3]](#references).
...
- **Per-source `holdStatus` regression** — a location that was `applied` and later shows
  ...
  Microsoft's "Hold changed outside eDiscovery" error entry [[6]](#references). Treat this the
...
- **A SharePoint site without a title cannot be placed on hold at all** — Microsoft's own guidance
  states this as a hard requirement [[7]](#references).
