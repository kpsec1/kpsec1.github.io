...
- The `purgeData` Graph reference itself states: **"When purgeType is set to either `recoverable` or
 ...
 deleted."**, both enum values, not just `permanentlyDelete`.
- "Find and delete Microsoft Teams chat messages in eDiscovery" draws the actual distinction: it's
 ...
 and can't be recovered for user use".
...
2. **Script the current, non-retired mechanism**, Microsoft Graph's `ediscoverySearch`/`purgeData`
 ...
 mailbox sibling ([Automation surface §3](/docs/automation-surface/#3-authentication-patterns-interactive-vs-unattended): app-only auth for eDiscovery Security &
...
5. **Disclose the hold-removal requirement as a real operational step, not a silent skip.** Unlike
 ...
 these holds, the chat messages you're trying to delete are retained". This
...
Microsoft's own data-source table for this exact workflow:
...
**all members** of the private channel", which does not obviously match "a
...
private-channel-search workflow, not just the "dedicated mailbox" claim alone.
...
group, and channel chats. The condition-builder reference's **Message kind**
condition documents the equivalent KQL-level value as `microsoftteams`
, this scenario's sample `contentQuery` therefore leads with
...
). Narrow further with a date range or distinctive keyword, the same tuning
...
, via `New-MgSecurityCaseEdiscoveryCaseNoncustodialDataSource`.
2. Bind those sources onto the search at creation time via the `noncustodialSources@odata.bind`
 request-body property, or after creation via
 `POST.../searches/{id}/noncustodialSources/$ref` for a mailbox added on a
 later run.
...
- **Teams Connect Chat (external access/federation) conversations**, Microsoft states search and
 purge isn't supported for these; not scriptable, not attempted.
- **Chats with yourself**, explicitly unsupported for search and delete.
- **The newer Data Security Investigations purge-queue workflow** Microsoft references as an
 alternative entry point, a different product surface this build didn't ground;
