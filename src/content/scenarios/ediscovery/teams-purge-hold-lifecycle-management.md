...
fullTitle: "eDiscovery — Teams-Purge Hold Lifecycle Management"
category: "eDiscovery"
...
[[1]](#references). Conversely, forgetting to **reapply** a removed hold is a preservation-duty gap:
...
Full licensing detail: `docs/licensing-matrix.md` §2 (eDiscovery — the parent scenario's own
entitlement covers this companion; no separate license is required for the Exchange Online/Security &
Compliance PowerShell surfaces this scenario uses). RBAC: `docs/rbac-model.md` §4 (eDiscovery role
...
| Role — remove/restore Litigation Hold and delay holds | Exchange Online **Legal Hold** role (assigned via **Organization Management** or **Discovery Management** by default) [[2]](#references) | A pure Exchange Online RBAC role, not a Purview role group — `docs/rbac-model.md` §6 |
| Role — remove/restore retention-policy membership | Purview **Retention Management** role, part of the **Records Management** role group [[3]](#references) | `docs/rbac-model.md` §4 already documents this role group for Records Management/Data Lifecycle Management |
| Graph/eDiscovery-hold resolution | **Not attempted by this scenario** — `Get-CaseHoldPolicy`/`Get-ComplianceCase` are eDiscovery cmdlets, the one documented app-only-unsupported exception (`docs/automation-surface.md` §3) | design.md §6 |
| Auth | Certificate-based app-only via **both** `Connect-ExchangeOnline` and `Connect-IPPSSession` | `docs/automation-surface.md` §3, surfaces 1 and 2 |
...
"Find and delete Microsoft Teams chat messages in eDiscovery" [[1]](#references), which itself points
to **eDiscovery → Cases → [case] → Hold policies** [[4]](#references) for eDiscovery case holds
...
3. Records Management role group (Retention Management role) — see `docs/rbac-model.md` §4; Microsoft
