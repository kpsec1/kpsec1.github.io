...
fullTitle: "eDiscovery — Microsoft Teams / Microsoft 365 Group Hold-Location Resolution"
category: "eDiscovery"
...
surface 1 per `docs/automation-surface.md` §1) — the only surface that exposes a Team/Microsoft 365
...
1. **Get the group's mailbox and site**: Purview eDiscovery hold-creation flow itself resolves a
   ...
   name) [[3]](#references). This scenario's script exists for the **automation** path, where a
...
2. **Confirm via Exchange Online PowerShell** (equivalent to what the script does): `Get-UnifiedGroup
   "<Team name>" | FL DisplayName,Alias,PrimarySmtpAddress,SharePointSiteUrl` [[1]](#references).
...
|---|---|
| `groups[].identity` | Any value `Get-UnifiedGroup -Identity` accepts — display name, alias, or SMTP address [[1]](#references) |
...
`SharePointSiteUrl` [[1]](#references). Graph endpoints used by `-AddToHold` (identical to the
sibling scenario's — full grounding there): `POST .../legalHolds/{id}/userSources`,
`POST .../legalHolds/{id}/siteSources` [[4]](#references)[[5]](#references).
...
2. **Functional proof (portal)** — after `-AddToHold`, open the case's **Hold policies** tab and
   ...
   sibling scenario's own §7 describes [[6]](#references).
3. **Member-roster sanity check** — if `resolveMembers: true` was used, the emitted CSV's row count
   ...
   membership [[7]](#references) — a mismatch suggests a stale cached result rather than a script
...
- **Placing a group's mailbox/site on hold does not cover its members' 1:1/1:N chats or personal
  ...
  hold" [[3]](#references)[[6]](#references). This scenario's `-ResolveMembers` output is a
...
- **This scenario does not resolve a Team channel down to individual private channels.** Private
  ...
  [[3]](#references) — out of scope here; see `design.md` §6.
