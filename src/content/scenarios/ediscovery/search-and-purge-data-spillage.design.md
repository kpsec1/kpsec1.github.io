...
. This scenario builds that tool as code: create a search, review
...
1. **Script the current mechanism, not the retired one.** Microsoft retired all classic eDiscovery
 ...
 hosted by 21Vianet (China). This scenario is built entirely
...
, the same object model this repo's
 `premium-legal-hold-and-export` sibling already uses for holds and exports.
2. **Follow this repo's own automation-surface guidance.** [Automation surface §3](/docs/automation-surface/#3-authentication-patterns-interactive-vs-unattended) already
...
) is the default; `PermanentlyDelete` requires an explicit switch and a second,
 ...
 **not** a hold-overriding control, Microsoft's own FAQ states that on a mailbox with a litigation
 ...
 aren't permanently deleted", and separately recommends **Priority Cleanup** as
 the tool for removing held content. This scenario's README states that limit
...
configured, not by which API the operator personally prefers:
...
ceiling, and requires **eDiscovery (Premium)** licensing ([Licensing matrix §2](/docs/licensing-matrix/#2-master-capability--license-matrix)), the same
...
- **`purgeData` is deliberately NOT idempotent-by-skip.** Unlike this repo's `addToReviewSet`/export
 ...
 only removed by repeating the purge. `Invoke-DataSpillagePurge.ps1` therefore
...
> cleanup policy to permanently delete the soft deleted items." 
...
). This scenario's README §5 documents that two-step sequence explicitly rather
...
|---|---|---|
| API surface | Microsoft Graph (`ediscoveryCase`/`ediscoverySearch`/`purgeData`), not S&C PowerShell | [Automation surface §3](/docs/automation-surface/#3-authentication-patterns-interactive-vs-unattended): app-only auth for eDiscovery S&C PowerShell cmdlets is unsupported by Microsoft |
...
- **Microsoft Teams message purge** (`purgeAreas: teamsMessages`), a materially different capability
 documented on its own page, now built as `scenarios/ediscovery/
