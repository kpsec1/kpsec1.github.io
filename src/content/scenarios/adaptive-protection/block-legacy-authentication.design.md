...
**separate, dedicated "block legacy authentication" policy**, a general
...
1. **Close the exact gap the sibling scenario flagged**, using Microsoft's own documented
 ...
 Target resources/Client apps/Grant/Report-only sequence, reproduced exactly
...
. A scenario that blindly creates a second, functionally identical policy
...
3. **Still offer a legitimate custom-policy path.** Microsoft's own guidance explicitly suggests
 ...
 than what's allowed in the Microsoft-managed policies", so detecting the
...
- Eligibility requires **Microsoft Entra ID P2** or **Microsoft 365 Business Premium** licensing
 per that page's own Prerequisites section, a narrower population than every
...
Microsoft 365 Business Premium), materially cheaper than the P2 floor its two
CA siblings require. [Licensing matrix §9](/docs/licensing-matrix/#9-adjacent-product-family-microsoft-entra-id-p1-baseline-conditional-access-block-legacy-authentication) documents this distinction so a buyer doesn't
...
policy at all, Conditional Access itself requires at least P1. **Security defaults**
 is Microsoft's documented zero-cost alternative for that tier: a single
...
|---|---|---|
| Check for a Microsoft-managed equivalent first | Best-effort `Microsoft-managed:` displayName prefix + `legacy` substring match, non-blocking (informational stop, not an error) | The only confirmed, citable Graph-visible signal for a Microsoft-managed policy is its naming convention, no documented boolean flag exists on the v1.0 `conditionalAccessPolicy` resource (checked directly against the resource reference). Disclosed as best-effort, not asserted as exhaustive, see §7 Non-goals and README.md §11. |
| `clientAppTypes` values | `['exchangeActiveSync', 'other']`, NOT `'all'`, `'browser'`, or `'mobileAppsAndDesktopClients'` | Matches Microsoft's own documented portal procedure exactly: "Check only the boxes Exchange ActiveSync clients and Other clients", confirmed against the `conditionalAccessConditionSet` resource's `clientAppTypes` enum. Scoping to only these two client types (rather than blocking all client apps) is what makes this a *legacy-authentication*-specific control instead of a blanket sign-in block, the latter is a materially different, much broader control this scenario deliberately does not build. |
...
| Target resources | `includeApplications = ['All']` | Matches Microsoft's own documented procedure step exactly. |
| Users scope | `includeUsers = ['All']` minus `-ExcludeUserIds`/`-ExcludeGroupIds` | Matches Microsoft's documented procedure (include all users, exclude break-glass/emergency-access and service accounts still dependent on legacy protocols). |
| Grant control | `builtInControls = ['block']`, `operator = 'OR'` | Matches Microsoft's documented "Block access" grant control choice exactly. |
...
- **This scenario does not configure or check Security defaults.** A separate, simpler, zero-cost
 ...
 policies), and out of this scenario's Conditional-Access-specific scope. See
 §4.
...
), a separate, workload-specific control surface Conditional Access's own
...
- **This scenario does not script Conditional Access for workload identities / service
 ...
 policy like this one's, a distinct, separate Conditional Access surface
...
at this point the attacker knows credentials were successfully verified". This scenario's block
