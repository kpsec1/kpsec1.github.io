...
fullTitle: "Adaptive Protection — Dynamic Risk-Based DLP Enforcement"
...
Full licensing detail and citations: `docs/licensing-matrix.md` §2 (Adaptive Protection and its
constituent DLP/Insider Risk Management rows). Summary for this scenario:

| Requirement | Minimum | Notes |
|---|---|---|
| Adaptive Protection (DLP/label enforcement) | **Microsoft 365 E5**, **Purview Suite**, or the underlying IRM + DLP add-ons | Built on Insider Risk Management + DLP — inherits both prerequisites [[10]](#references) |
...
| DLP for Exchange and Teams | Advanced classification & Teams-scoped DLP → **E5** | `docs/licensing-matrix.md` §2, DLP row |
...
| Role to create/manage the DLP policy in this scenario | One of: **Compliance Administrator**, **Compliance Data Administrator**, **DLP Compliance Management**, **Global Administrator** | [[2]](#references) — same DLP-authoring roles used elsewhere in this library, see `docs/rbac-model.md` |
| Automation identity for the deploy script | App-only certificate authentication to Security & Compliance PowerShell | `docs/automation-surface.md` §3 — same certificate app-only pattern as every other DLP scenario in this library |
...
**Compliance Data Administrator**, or **DLP Compliance Management** [[2]](#references).
...
[[4]](#references), not a fabricated default. Confirm the feeder policy from Step 1 is selected
on this tab.
...
[[1]](#references) before expecting risk levels to be assigned and this scenario's DLP rules to
...
| Initial policy mode | `TestWithNotifications` (simulation) | Matches Microsoft's own Quick Setup default — every rule Microsoft's wizard creates starts this way [[3]](#references) |
| Incident report severity | `Low` for both rules | Matches Microsoft's documented values [[3]](#references) |
| User override | Not enabled (default off) | Matches Microsoft's documented values — a flagged-elevated user cannot self-override the block [[3]](#references) |
| Insider risk level definitions | Elevated = confirmed alert of any severity; Moderate = high-severity alert; Minor = low/medium-severity alert | Microsoft's own "minimize false positives" recommended starting configuration [[4]](#references) — a starting point, tune per §8 |
| Past activity detection window | 7 days (Microsoft default) | Configurable 5–30 days in Adaptive Protection settings; not overridden by this scenario [[1]](#references) |
| Insider risk level timeframe (auto-reset) | 7 days (Microsoft default) | How long a level stays assigned before automatically resetting, absent a new qualifying event [[1]](#references) |
| Adaptive Protection propagation delay | Up to 36 hours after first enabling | Backend processing delay in the Adaptive Protection service itself — not scriptable around [[1]](#references) |
...
- **No incremental license cost for a tenant already at E5/Suite for the feeder IRM policy and
  ...
  entitlements, not a separate SKU (`docs/licensing-matrix.md` §2, Adaptive Protection row
  [[10]](#references)).
...
10. `docs/licensing-matrix.md` §2 — Adaptive Protection row (E5/Suite, built on IRM + DLP)
