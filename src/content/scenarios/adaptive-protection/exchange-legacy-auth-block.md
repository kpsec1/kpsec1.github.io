...
whoFor: "any Exchange Online tenant that wants legacy-authentication coverage that"
...
. The controls in this scenario are evaluated inside the Exchange Online
...
| Role to run the deploy/validate scripts interactively, or to assign to the app's service principal for unattended use | **Organization Management** (Exchange Online role group) | Confirmed sufficient by this build (it is Exchange Online's superset administrative role group); the exact least-privilege granular Exchange management role scoped to just Authentication Policies is not independently confirmed, see §11 and [RBAC model §6](/docs/rbac-model/#6-exchange-online-dependency-the-most-common-permissions-gap). |
...
Management** Exchange Online role group, [RBAC model §6](/docs/rbac-model/#6-exchange-online-dependency-the-most-common-permissions-gap). See §11 for the narrower-role
open question.
...
. **Creating this policy alone has zero effect on any user**, an
...
- **No incremental license.** Authentication Policies, `Set-TransportConfig`, and `Set-CASMailbox`
 ...
 P1/P2. [Licensing matrix §4](/docs/licensing-matrix/#4-common-cross-module-prerequisites).
...
- **Time-boxed urgency, not a discretionary nice-to-have.** Microsoft's own SMTP AUTH default-
 disable date (end of December 2026) means a buyer who does nothing gets a
...
14. [Automation surface §1/§3](/docs/automation-surface/#1-five-automation-surfaces-not-one-read-this-first), automation surface 1 (Exchange Online PowerShell),
 app-only authentication pattern.
15. [RBAC model §6](/docs/rbac-model/#6-exchange-online-dependency-the-most-common-permissions-gap), Exchange Online RBAC dependency.
