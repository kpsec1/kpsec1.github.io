...
fullTitle: "Adaptive Protection — Direct Send and Anonymous Relay Hardening"
...
- **Direct Send is unauthenticated by design, and Microsoft's own guidance says most tenants don't
  ...
  on the responsibilities of email server admins" [[1]](#references). No credential, certificate, or
...
- **Microsoft has itself flagged Direct Send as a default-posture risk.** The same reference page
  ...
  Direct Send by default to protect customers" [[1]](#references) — this scenario deploys the
...
- **A documented, real-world abuse pattern.** Direct Send messages sent to internal recipients with
  ...
  on intra-org spoofing detection (`compauth=fail reason=6xx`) [[8]](#references).
- **Anonymous relay connectors are a separate, complementary gap.** An `InboundConnector` configured
  ...
  organization" [[2]](#references) precisely because a shared or overly broad IP range defeats the
  ...
  which Microsoft documents as not requiring a static IP at all [[2]](#references)[[5]](#references).
...
Management** Exchange Online role group — `docs/rbac-model.md` §6.
...
[[6]](#references) — this **never blocks or modifies mail**, it only tags matching messages and logs
...
Creates a certificate-authenticated `InboundConnector` [[2]](#references)[[5]](#references) —
...
[[2]](#references). Run once per distinct sending application/device family that needs to keep
...
Sets `Set-OrganizationConfig -RejectDirectSend $true` [[3]](#references) — Exchange Online rejects
...
path Microsoft's own comparison table distinguishes from Direct Send [[1]](#references)). Any
...
|---|---|---|
| Audit rule name | `Direct Send Detection (Audit)` | `-Mode Audit` — never blocks; tags matches with `X-DirectSendHardening-Detected: True` [[6]](#references). |
...
- **This scenario does not detect or block internal-sender spoofing that arrives via an already-
  ...
  [[8]](#references)), a separate control this scenario does not duplicate or replace.
- **The audit-mode transport rule detects Direct Send traffic aimed at internal recipients only**
  ...
  [[1]](#references) — it will not surface anonymous-relay abuse aimed at external recipients via an
...
- **A certificate-based relay connector (Step 4) is not itself internal-recipient-only** — unlike
  ...
  address and, depending on configuration, external recipients [[1]](#references). This is a
...
- **Grounding note:** `techcommunity.microsoft.com` (the Exchange Team's own "What is Direct Send and
  ...
  [[1]](#references), the `Set-OrganizationConfig`/`New-InboundConnector`/`New-TransportRule` cmdlet
