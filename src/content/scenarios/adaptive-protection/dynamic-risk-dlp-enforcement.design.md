...
2. **Script the part that has a real, grounded PowerShell surface, and only that part.**
 ...
 is" condition exposed in the portal. This is a
...
. This scenario's deploy script reproduces that same two-rule shape
...
, not enforcing on day one. This scenario's script defaults to the same
...
5. **Scope narrowly to what's grounded; name what's deferred.** Adaptive Protection also
 supports **Devices** (Endpoint DLP) and **Conditional Access** locations 
. Both are explicitly out of scope for this fragment, see §7 (Non-goals)
...
. A human is still very much in the loop for
...
| DLP policy + rules using the risk-level condition | `deploy/New-AdaptiveProtectionDlpPolicy.ps1` → `New-/Set-DlpCompliancePolicy`, `New-/Set-DlpComplianceRule -SharedByIRMUserRisk` | **Yes**, this scenario's code, independently grounded |
...
Elevated/Moderate/Minor conditions. This scenario's deploy script never reads
...
. This scenario's deploy script has no way to script around that, it is a
...
|---|---|---|
| Locations covered | **Exchange + Teams only** (one combined DLP policy) | Matches Microsoft's own "Adaptive Protection policy for Teams and Exchange DLP" Quick Setup output exactly; Endpoint DLP (Devices) requires an additional, separately-gated prerequisite (Advanced classification scanning and protection, or an explicit File Type condition) that this fragment defers rather than bundles in, see §7. |
| Rule shape | Two rules: **Elevated → block external share**; **Moderate + Minor → audit external share** | This is Microsoft's own documented, tested default configuration for the Teams/Exchange policy, reproduced exactly rather than inventing a different threshold scheme, so a buyer evaluating this scenario can cross-check it directly against Microsoft's own docs. |
| Condition parameter | `New-DlpComplianceRule -SharedByIRMUserRisk <GUID[]>` | The only Security & Compliance PowerShell parameter found and independently confirmed to implement the portal's "Insider risk level for Adaptive Protection is" condition, with three fixed, documented GUID values for Elevated/Moderate/Minor, not fabricated or inferred by analogy. |
| Deployment path | **Custom setup**, not Quick Setup | Quick Setup bundles a new auto-created IRM policy, Conditional Access policy, and Data Lifecycle Management policy into one irreversible-feeling wizard action, wrong fit for a buyer using this library's own IRM scenario as the feeder policy, or who wants to review/approve each control independently. This scenario's script is the scriptable equivalent of the portal's documented "Manual configuration" / Custom setup Step 3. |
| Initial policy mode | `TestWithNotifications` (simulation), matching Microsoft's own Quick Setup default | Every rule Microsoft's own Quick Setup creates starts in simulation mode, this scenario doesn't go further/faster than Microsoft's own recommended default, especially since a wrongly-tuned Elevated-risk block rule has direct, immediate business impact on a real (if risky) employee's ability to work. |
...
- **This scenario does not configure Endpoint DLP (Devices) risk-based restrictions.** The
 ...
 a prerequisite, and its restriction-action parameter
...
- **This scenario does not configure the Conditional Access "Insider risk" condition.** That's a
 ...
 this scenario's DLP-only design doesn't otherwise require. Built as the
 ...
 the sibling's own citation) during this correction pass.
- **This scenario does not configure the Data Lifecycle Management 120-day deleted-content
 preservation policy** (also a Microsoft-labeled **preview** integration)
