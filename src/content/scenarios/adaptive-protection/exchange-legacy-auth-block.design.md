...
. That scenario's own `design.md` §7 names the mitigation and defers it here:
...
1. **Close the exact gap the sibling scenario disclosed**, using Exchange Online's own documented
 Authentication Policy mechanism, not a Conditional Access
 workaround.
...
Autodiscover, and Outlook for Windows/Mac (MAPI/RPC). This happened
...
|---|---|---|---|
| Authentication Policy | `AllowBasicAuthSmtp` on an assigned `AuthenticationPolicy` | Per-user (via the policy assigned to them, or the org default) | |
| Transport config | `SmtpClientAuthenticationDisabled` on `Set-TransportConfig` (org-wide) / `Set-CASMailbox` (per-mailbox override) | Org-wide with mailbox-level override; `$null` on the mailbox means "follow the org setting" | |

Microsoft's own dedicated SMTP AUTH guide documents the transport-config gate's
org-then-mailbox-override precedence in detail, but neither that page nor the
`Set-AuthenticationPolicy` reference states what happens if, say, a mailbox's
...
|---|---|---|
| Baseline policy's `AllowBasicAuth*` values | All left at default (blocked), no switches passed to `New-AuthenticationPolicy` | Matches Microsoft's own confirmed default behavior for a policy created with no `-AllowBasicAuth*` switches; the simplest, most conservative baseline. |
| Deployment path | Direct Exchange Online PowerShell (`New-/Set-AuthenticationPolicy`, `Set-OrganizationConfig`, `Set-TransportConfig`, `Set-CASMailbox`) | This is automation surface 1 per [Automation surface §1](/docs/automation-surface/#1-five-automation-surfaces-not-one-read-this-first), the only surface these cmdlets exist on; no Graph equivalent found during this build's grounding pass. |
...
- **This scenario does not attempt on-premises/hybrid Exchange's `-BlockLegacyAuth*`/
 `-BlockModernAuth*` parameter family**, those parameters are documented as
...
- **No Microsoft-side audit trail exists for a rejected SMTP AUTH attempt, a grounded, structural
 ...
 pre-authentication step) capture this event, and the Exchange SMTP
