...
whoFor: "any enterprise whose regulated population is EU/UK-only (or includes a"
...
| Role to author/edit auto-labeling policies | **Information Protection Admin** role group | [RBAC model §3](/docs/rbac-model/#3-microsoft-entra-roles-that-map-into-purview) |
...
[Automation surface §1](/docs/automation-surface/#1-five-automation-surfaces-not-one-read-this-first).
...
. A file updated after the last simulation pass won't show until the next
 one.
...
events via the Audit Search/Graph pattern in [Automation surface §4](/docs/automation-surface/#4-routing-table-which-surface-for-which-purview-task) for SIEM integration.
...
- **No PAYG component for M365 content.** Same per-user E5-tier/IP&G-add-on entitlement as the
 sibling scenario, see [Licensing matrix §1](/docs/licensing-matrix/#1-the-two-billing-models-read-this-first), 2. No incremental licensing cost for
...
- **The EU-wide bundle SITs cannot be copied or edited.** Microsoft's own documentation lists "EU
 ...
 source for a custom SIT. If a buyer needs a *modified* version of one of
